...
function handleFsEvent(obj)
{
    ...
    if (zonexml_re.test(obj.pathname)) {
        var matches = zonexml_re.exec(obj.pathname);
        ...
        event_queue.enqueue(zonename, function (cb) {
            ...
            if (obj.type == 'delete') {
                ...
                async.series([
                    ...
                    function (cb) {
                        for (key in vm_data) {
                            if (vm_data[key].hasOwnProperty(zonename)) {
                                delete (vm_data[key])[zonename];
                            }
                        }
                        cb();
                    }
                ...
                contexts.forEach(function (context) {
                    if (vm_data.hasOwnProperty(context) && 
                        vm_data[context].hasOwnProperty(zonename)) {

                        delete (vm_data[context])[zonename];
                    }
                });
        ...
        var matches = metadata_re.exec(obj.pathname);
        var zonename = matches[1];
...
function handleZpoolEvent(obj)
{
    ...
    function expire() {
        var ds_objects = vm_data.dataset_objects;
        sections.forEach(function (section) {
            if (ds_objects.hasOwnProperty(section)) {
                if (section === 'mountpoints') {
                    for (key in ds_objects[section]) {
    ...
    function copy(cache) {
        sections.forEach(function (section) {
            for (key in cache[section]) {
                if (!vm_data.hasOwnProperty('dataset_objects')) {
                    vm_data.dataset_objects = {};
                }
                if (!vm_data.dataset_objects.hasOwnProperty(section)) {
                    vm_data.dataset_objects[section] = {};
                }
                vm_data.dataset_objects[section][key] = cache[section][key];
            }
        });
    }
...
{
    event_queue = new Queue({workers: 1});
    callback();
}
