 ...
 * XXX TODO
 *
 *  - if watcher restarts, can we somehow pass in the timestamp of the last
 *    event so that we catch any changes that might have happened while it was
 *    not running?
 */
...
var DEDUP_FUDGE = 2; // ms +/- to consider "same" when checking dupes
...
function FsWatcher(log)
{
     ...
     * within DEDUP_FUDGE ms, returns true. If it's a different event or spaced
     * further apart, returns false.
     */
    function isDuplicate(obj) {
         ...
         */
        if ((self.prev_event.timestamp + DEDUP_FUDGE < obj.timestamp)
            || (self.prev_event.timestamp - DEDUP_FUDGE > obj.timestamp)) {
     ...
     */
    self.queueAction = function (pathname, action, options, callback) {
        var wobj;

        wobj = {pathname: pathname, action: action};
        if (options && options.timestamp && options.timestamp > 0) {
            assert(action === 'WATCH'); // timestamp makes no sense elsewhere
            wobj.timestamp = options.timestamp;
        }
        if (callback) {
                    ...
                    Object.keys(children).forEach(function (child) {
                        files.forEach(function (file) {
                            ...
                            if ((child.substr(0, file.length) === file)
                                ...
                                log.debug(child + ' might be under ' + file + ' deleting and reparenting');
                                ...
                                self.queueAction(child, 'WATCH');
                            } else {
                                log.debug(child + ' definitely not under ' + file);
                            }
                        });
                    });
                });
    ...
    self.watch_queue = async.queue(function (obj, callback) {
        ...
        if (action === 'UNWATCH') {
            ...
            try {
                ...
                log.error({watch_err: e}, 'failed to write WATCH: ' + e.message);
            }
        }
