...
function handleFsEvent(obj)
{
    ...
    if (zonexml_re.test(obj.pathname)) {
        ...
        event_queue.enqueue(zonename, function (callback) {
        ...
        event_queue.enqueue(zonename, function (cb) {
            // short-circuit if this vmobj doesn't exist
            if (!vmobjs.hasOwnProperty(zonename)) {
                log.debug('vmobj doesn\'t exist for ' + zonename);
                cb();
...
function handleZpoolEvent(obj)
{
    ...
    // to the order of operations because 1) we use the dataset
    // name as the channel so they're not actually blocking here
    // and 2) vmload will serialize all zfs operations anyway.
    event_queue.enqueue(name, function (cb) {
