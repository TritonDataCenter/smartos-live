    ...
    async.waterfall([
        function (cb) {
            ...
            fs.mkdir(snapdir, newmode, function _mkSnapshotsDir(e) {
                 ...
                 */
                fs.writeFile(snapfile, data,
                    function _writeSnapshotFileCB(err) {
                        if (err) {
                        ...
                        log.debug({data: data}, 'wrote dataset info to '
                            + snapfile);
                        cb();
                    }
                );
            ...
            /*
             * If VM is running and supports mounting of snapshots (checked via
             * proptable features), attempt to mount the snapshot of zoneroot on
             * /checkpoints
             */

            if (!BRAND_OPTIONS.hasOwnProperty(vmobj.brand)) {
                log.warn('not mounting snapshots for VM ' + vmobj.uuid
                    + ', unknown brand "' + vmobj.brand + '"');
                cb();
                return;
            }

            if (!BRAND_OPTIONS[vmobj.brand].hasOwnProperty('features')
                || !BRAND_OPTIONS[vmobj.brand]
                .features.mount_zoneroot_snapshot) {

                log.info('not mounting snapshots for VM ' + vmobj.uuid
                    + ', brand does not support "mount_zoneroot_snapshot" '
                    + 'feature');
                cb();
                return;
            }
