...
exports.delete_snapshot = function (uuid, snapname, options, callback)
{
    ...
    vasync.waterfall([
        function (cb) {
            ...
            if (vmobj.brand === 'kvm') {
                cb(new Error('snapshots for KVM VMs currently unsupported'));
                return;
            }
            ...
            vasync.parallel({funcs: [
                function (cb2) {
                    var now = new Date(Math.floor(Date.now() / 1000)
                        * 1000);
                    var obj = {
                        uuid: uuid
                    };
                    var changes = [];
                    if (new Date(vmobj.last_modified) < now) {
                        changes.push({
                            path: ['last_modified'],
                            action: 'changed'
                        });
            ...
            vasync.parallel({funcs: [
                ...
                function (cb2) {
                    ...
                    updateZonecfgTimestamp(vmobj, function (e) {
                        if (e) {
                            log.warn({err: e},
                                'failed to update timestamp after '
                                + 'snapshot');
                            cancelFn();
        ...
        if (options.hasOwnProperty('value')) {
            ...
            if (im['docker:restartcount']) {
                im['docker:restartcount'] =
                    jsprim.parseInteger(im['docker:restartcount'])
                    + options.increment;
            } else {
                ...
                if ((m = key.match(/^update_(.*)$/))) {
                    ...
                    value.forEach(function (thing) {
                        ...
                        Object.keys(thing).forEach(function (_key) {
                            ...
                            localChange.newValue = thing[_key];
