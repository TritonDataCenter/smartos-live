        ...
        try {
            ...
            if (stat.isFile()) {
                if (stat.mtime > timestamp) {
                    VM.log('DEBUG', 'getLastModified() found ' + stat.mtime
                        + 'for ' + file);
        ...
        loadMetadata(vmobj, function (error, metadata, tags) {
            ...
            if (vmobj.state === 'receiving') {
                vmobj.missing = { 'datasets': [], 'disks': [],
                    'filesystems': [] };
                if (!path.existsSync(vmobj.zonepath)) {
            ...
            if (volume.hasOwnProperty('image_uuid')) {
                ...
                zfs(['snapshot', snapshot], function (err, fds) {
                    if (err) {
                        ...
                        zfs(['clone', '-F', snapshot, target], function (e) {
                            if (e) {
                                cb(e);
                            } else {
...
function checkPayloadProperties(payload, vmobj, callback)
{
    ...
    if (!payload.hasOwnProperty('transition')
        ...
        for (filesys in payload.filesystems) {
            ...
            if (!path.existsSync(filesys.source)) {
...
function createDelegatedDatasets(payload, callback)
{
    if (!payload.hasOwnProperty('datasets')) {
        payload.datasets = [];
    }

    async.forEachSeries(payload.datasets, function (ds, cb) {
        zfs(['create', '-o', 'compression=on', ds], function (err) {
            if (err) {
                cb(err);
                return;
            }
...
function installZone(payload, callback)
{
    ...
    async.series([
        function (cb) {
            ...
            if (vmobj.brand === 'joyent' && !receiving) {
            ...
            if (payload.brand === 'joyent' && !receiving) {
                createDelegatedDatasets(payload, function (err) {
                    if (err) {
                        cb(err);
                    } else {
                        cb();
...
function createZone(payload, callback)
{
    ...
    payload.zfs_filesystem = payload.zpool + '/'
        + payload.zonename;
    ...
    // add delegated dataset(s)
    delegated_ds = payload.zfs_filesystem + '/data';
    if (!payload.hasOwnProperty('datasets')) {
        payload.datasets = [];
    }
    if (payload.delegate_dataset && !payload.datasets.indexOf(delegated_ds)) {
        payload.datasets.push(delegated_ds);
    }
    for (ds in payload.datasets) {
        ds = payload.datasets[ds];

        VM.log('DEBUG', 'adding delegated dataset ' + ds + 'to '
            + 'VM ' + payload.uuid);
        ...
        for (n in payload.add_nics) {
            if (payload.add_nics.hasOwnProperty(n)) {
                ...
                if (nic.hasOwnProperty('primary')
                    ...
                    nic.primary = true;
                } else if (nic.hasOwnProperty('primary')
                    && nic.primary === false) {
...
function normalizePayload(payload, vmobj, callback)
{
    ...
    // fix type of arguments that should be numbers
    for (property in payload) {
        if (payload.hasOwnProperty(property)) {
            if (INTEGER_PROPERTIES.indexOf(property) !== -1) {
    ...
    // Can't include spice_* options if no zone_dataset_uuid on create
    if ((brand === 'kvm' && !vmobj
        && !payload.hasOwnProperty('zone_dataset_uuid'))
        || (brand !== 'kvm')) {
        ...
        if (payload.hasOwnProperty('spice_opts')
            ...
            VM.log('WARN', 'SPICE options require brand == KVM and '
                + ' zone_dataset_uuid, REMOVING');
