...
function updateMetadata(vmobj, payload, log, callback)
{
    ...
    mdata = {
        customer_metadata: cmdata,
        internal_metadata: imdata
    };
    ...
    vs.on('ready', function (ready_ev) {
        ...
        assert.object(latest_vmobj, 'vminfod latest_vmobj updateMetadata');
        ...
        Object.keys(data).forEach(function (_key) {
            if (deepEqual(data[_key], latest_vmobj[_key]))
                delete data[_key];
        });
        ...
        vasync.parallel({funcs: [
            function (cb) {
                ...
                cancel = vs.watchForEvent(obj, opts, cb);
            },
            function (cb) {
                async.series([
                    function (cb2) {
                        writeAndRename(log, 'metadata', mdata_filename,
                            JSON.stringify(mdata, null, 2), cb2);
                    },
                    function (cb2) {
                        writeAndRename(log, 'tags', tags_filename,
                            JSON.stringify(tags, null, 2), cb2);
                    }
                ], function (err) {
                    if (err) {
                        if (cancel)
...
exports.receive = function (target, options, callback)
{
    ...
    vasync.pipeline({funcs: [
        function (_, cb) {
            receiveStdinChunk('JSON', {log: log}, function (error, _result) {
                if (error) {
                    cb(error);
                    return;
                }
            ...
            // The event stream is used when vmbundle is forked with the
            // DATASET option.  This way, we can properly block on vminfod
            // to see the disks.* added event.
            vs = new vminfod.VminfodEventStream({
                name: sprintf('VM.js receive (%s)', result.uuid),
                log: log
            });
            ...
            async.whilst(
                ...
                function (cb2) {
                    ...
                    receiveStdinChunk('DATASET', opts, function (err, res) {
                        if (err) {
                            cb2(err);
                            return;
                        }
                    ...
                    VM.install(result.uuid, {log: log}, function (e) {
                        if (e) {
                            log.warn({err: e},
                                'couldn\'t install VM: %s', e.message);
                            cb(e);
                            return;
                        }
                ...
                vs.on('ready', function () {
                    vasync.parallel({funcs: [
                        function (cb2) {
                            ...
                            vs.watchForChanges(obj, changes, opts, cb2);
                        },
                        function (cb2) {
        ...
        function (cb) {
            ...
            keys.forEach(function (key) {
                ...
                if (IGNORE_KEYS.indexOf(key) > -1) {
                    log.debug({key: key}, 'key "%s" cannot be modified');
                    return;
                }
                ...
                // ram is a special key which results in us setting a number of
                // other keys (if not set explicitly in the original payload)
                if (key === 'ram') {
                    [
                        'max_physical_memory',
                        'max_locked_memory',
                        ...
                        if (newkey !== 'ram' && keys.indexOf(newkey) > -1) {
                            ...
                            // the "ram" key being set is set explicitly in the
                            // original payload - skip it
                            return;
                        }
