...
function addSnapshots(vmobj, dataset_objects, options)
{
    ...
    if (dataset_objects.snapshots.hasOwnProperty(vmobj.zfs_filesystem)) {
        raw_snapshots = dataset_objects.snapshots[vmobj.zfs_filesystem];
    }
     ...
     */
    raw_snapshots.forEach(function (snap) {
        var friendly_snap;
        ...
        if (matches && matches[1]) {
            ...
            log.debug('ignoring snapshow w/o vmsnap- prefix: ' + snap.snapname);
        }
    });
...
/*
 * KVM VMs keep a pidfile for the qemu process. We include this in VM objects
 * as the 'pid' property when the VM is running. This function reads and
 * returns the PID assuming it's safe to do so.
 */
function getPid(zonepath, pidfile, options, callback)
{
    ...
    /*
     * zonepath really should always be safe for KVM but we check here anyway
     * just to be sure.
     */
    try {
    ...
    log.trace('looking for KVM PID in ' + filename);
    ...
    fs.readFile(filename, function (error, filedata) {
        ...
        log.trace('found PID ' + pid + ' in ' + filename);
        callback(null, pid);
    });
}
...
function getVmobj(uuid, options, callback)
{
    ...
    async.series([
        function (cb) {
            ...
            if (!BRAND_OPTIONS.hasOwnProperty(vmobj.brand)) {
                log.warn('not loading PID for VM ' + vmobj.uuid + ', unknown '
                    + 'brand ' + vmobj.brand);
                cb();
                return;
            }
            ...
            if (cache.pids && cache.pids.hasOwnProperty(vmobj.uuid)) {
                vmobj.pid = cache.pids[vmobj.uuid];
            } else {
                log.trace('no PID in cache for ' + vmobj.uuid);
            }
            cb();
        }, function (cb) {
            /*
...
function loadPids(uuid, cache, options, callback)
{
    var log;
    var pids = {};
    var start_time;
    ...
    if (cache.hasOwnProperty('pids')) {
        // Already have pids passed in, trust caller
        log.trace('using existing cache.pids');
        callback();
        return;
    }

    if (!wantField(options, 'pid')) {
        log.trace('no need to load "pid", field not requested');
        callback();
        return;
    }
    ...
    // load PID files from any zone that needs it (in parallel)
    async.each(Object.keys(cache.zoneadm_objects),
        function (vm_uuid, cb) {
            ...
            if (!BRAND_OPTIONS.hasOwnProperty(obj.brand)) {
                log.warn('Skipping PID load for unkown brand ' + obj.brand);
                cb();
                return;
            }

            assert(BRAND_OPTIONS[obj.brand].hasOwnProperty('features'),
                'missing "features" for BRAND_OPTIONS[' + obj.brand + ']');
            features = BRAND_OPTIONS[obj.brand].features;

            if (!features.hasOwnProperty('pid_file')) {
                log.trace('no need to load "pid" for ' + vm_uuid + ', brand is '
                    + obj.brand);
                cb();
                return;
            }

            getPid(obj.zonepath, features.pid_file, options,
                function (err, pid) {
                    if (err) {
                        log.error(err, 'failed to load PID of ' + vm_uuid);
                        cb();
                        return;
                    }
            ...
            log.debug('loading PIDs took ' + ((new Date).getTime() - start_time)
                + ' ms');
