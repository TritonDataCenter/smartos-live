        ...
        function (_, cb) {
            ...
            vs.once('ready', function (ready_ev) {
                ...
                Object.keys(mdata).forEach(function (_key) {
                    if (deepEqual(mdata[_key], latest_vmobj[_key])) {
                        delete mdata[_key];
                    }
                });

                if (mdata.hasOwnProperty('customer_metadata')
                    && !mdata.hasOwnProperty('internal_metadata')) {
                    mdata.internal_metadata = latest_vmobj.internal_metadata
                        || {};
                } else if (mdata.hasOwnProperty('internal_metadata')
                    && !mdata.hasOwnProperty('customer_metadata')) {
                    mdata.customer_metadata = latest_vmobj.customer_metadata
                        || {};
                }

                log.debug('metadata modified: %s',
                    Object.keys(mdata).join(',') || '<none>');
                ...
                function (cb2) {
                    ...
                    vasync.pipeline({funcs: [
                        function (__, cb3) {
                            ...
                            if (mdata.hasOwnProperty('internal_metadata')) {
                            ...
                            // write routes
                            if (!mdata.hasOwnProperty('routes')) {
                                cb3();
                                return;
                            }
