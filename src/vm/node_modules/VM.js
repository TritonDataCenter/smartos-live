...
function updateMetadata(vmobj, payload, log, callback)
{
    ...
    log.debug('creating vminfod event stream');
    vs = new vminfod.VminfodEventStream({
        name: sprintf('VM.js updateMetadata (%s)', uuid)
    });
        ...
        vasync.parallel({
            funcs: [
                function (cb2) {
                    ...
                    log.debug({obj: obj},
                        'watching for vminfod event');
                    vs.watchForEvent(obj, opts, function (e, _) {
                        if (e) {
                            cb2(e);
                            return;
                        }

                        log.debug('got vminfod modify event');
                        cb2();
                    });
                },
                function (cb2) {
...
function updateRoutes(vmobj, payload, log, callback)
{
    ...
    log.debug('creating vminfod event stream');
    vs = new vminfod.VminfodEventStream({
        name: sprintf('VM.js updateRoutes (%s)', uuid)
    });
        ...
        vasync.parallel({
            funcs: [
                ...
                function (cb2) {
                    ...
                    writeAndRename(log, 'routes', filename, data, cb2);
                }
            ]
        }, function (err, results) {
            if (err) {
...
exports.waitForZoneState = function (payload, state, options, callback)
{
    ...
    log.debug('creating vminfod event stream');
    vs = new vminfod.VminfodEventStream({
        name: sprintf('VM.js waitForZoneState (%s)', payload.zonename)
    });
    ...
    vs.on('ready', function () {
        ...
        var opts = {
            timeout: options.timeout || VMINFOD_TIMEOUT,
            log: log,
            catchErrors: true,
            teardown: true
        };

        log.debug({obj: obj}, 'watching for vminfod event');
        timeout = vs.watchForEvent(obj, opts, function (e, _) {
            if (e) {
                done(e);
                return;
            }
        ...
        // after we've started the watcher (if we checked before there'd be a
        // race) we check whether we're already in the target state, if we are
...
function watchZoneTransitions(handler, log, cb) {
    assert(log, 'no logger passed to watchZoneTransitions()');

    log.debug('creating vminfod event stream');
    var vs = new vminfod.VminfodEventStream('VM.js watchZoneTransitions');
    ...
    vs.on('error', function (e) {
        log.error({err: e}, 'vminfod stream error');
        handler(e);
    });
...
function makeIndestructible(dataset, log, callback)
{
    ...
    zfs(args, log, function _makeIndestructibleSnapshot(snap_err, snap_out) {
        ...
        zfs(args, log, function _makeIndestructibleHold(hold_err, hold_out) {
            if (hold_err && !hold_out.stdout.match(hold_exists_pattern)) {
                callback(hold_err);
                return;
            }
...
function installZone(payload, log, callback)
{
    ...
    async.series([
        function (cb) {
            ...
            vs.on('ready', function (ready_ev) {
                log.debug('vminfod event stream ready');
                cb();
            });
        }, function (cb) {
            // Apply compression if set
            var key = 'zfs_root_compression';
            ...
            vasync.parallel({
                funcs: [
                    ...
                    function (cb2) {
                        var args = ['set',
                            'compression=' + payload.zfs_root_compression,
                ...
                funcs: [
                    function (cb2) {
                        var obj = {
                            zonename: payload.uuid,
                            vm: {
                                zfs_root_recsize: payload.zfs_root_recsize
                            }
                        };
                        var opts = {
                            timeout: VMINFOD_TIMEOUT,
                    ...
                    function (cb2) {
                        var args = ['set', 'recsize='
                            + payload.zfs_root_recsize, payload.zfs_filesystem];
                        zfs(args, log, function (err) {
                            cb2(err);
                        });
                    }
            ...
            vasync.parallel({
                funcs: [
                    ...
                    function (cb2) {
                        var args = ['set',
                            'filesystem_limit=' + payload.zfs_filesystem_limit,
                            payload.zfs_filesystem];
                        zfs(args, log, function (err) {
                            cb2(err);
                        });
            ...
            vasync.parallel({
                funcs: [
                    ...
                    function (cb2) {
                        var args = ['set',
                            'snapshot_limit=' + payload.zfs_snapshot_limit,
                            payload.zfs_filesystem];
                        zfs(args, log, function (err) {
                            cb2(err);
                        });
            ...
            vasync.parallel({
                funcs: [
                    ...
                    function (cb2) {
                        makeIndestructible(payload.zfs_filesystem, log, cb2);
                    }
                ]
            }, function (e) {
                    cb(e);
            ...
            vasync.parallel({
                funcs: [
                    ...
                    function (cb2) {
                        createDelegatedDataset(payload, log, cb2);
                    }
                ]
            }, function (e) {
                    cb(e);
            ...
            vasync.parallel({
                funcs: [
                    ...
                    function (cb2) {
                        makeIndestructible(payload.zfs_filesystem + '/data',
                            log, cb2);
                    }
                ]
            }, function (e) {
            ...
            funcs: [
                function (cb2) {
                    ...
                    vs.watchForEvent(obj, opts, function (e, _) {
                        ...
                        log.debug('got vminfod create event');
                        cb2();
                    });
                },
                function (cb2) {
                    ...
                    zonecfgFile(zcfg, ['-z', payload.zonename], log,
                        ...
                        if (err) {
                            ...
                            // don't wait for a vminfod event that won't come
                            vs.stop();
                            callback(err);
                            return;
                        }
    ...
    async.series([
        ...
        // TODO: replace these next two with VM.stop(..{force: true} ?
            ...
            var vs = new vminfod.VminfodEventStream({
                name: sprintf('VM.js deleteZone (%s)', vmobj.zonename)
            });
                    ...
                    funcs: [
                        function (cb2) {
                            ...
                            vs.watchForEvent(obj, opts, function (e, _) {
                                ...
                                log.debug('got vminfod delete event');
                                cb2();
                            });
                        }, function (cb2) {
        ...
        function (cb) {
            ...
            vasync.parallel({funcs: [
                function (cb2) {
                    ...
                    log.debug({obj: obj},
                        'watching for vminfod running event');
                    vs.watchForEvent(obj, _opts, function (e, _) {
                        if (e) {
                            cb2(e);
                            return;
                        }

                        log.debug('got vminfod running event');
                        cb2();
                    });
                }, function (cb2) {
            ...
            VM.waitForZoneState(vmobj, 'running',
                {timeout: 60 * 1000, log: log},
                function (err, result) {
                ...
                vs.on('ready', function () {
                    ...
                    vasync.parallel({funcs: [
                        function (cb2) {
                            ...
                            log.debug({obj: obj, changes: changes},
                                'watching for vminfod changes');
                            vs.watchForChanges(obj, changes, opts,
                                ...
                                log.debug('got vminfod changes event');
                                cb2();
                            });
                        },
                        function (cb2) {
...
exports.update = function (uuid, payload, options, callback)
{
    ...
    async.series([
        function (cb) {
            ...
            // we create the vminfod stream now and buffer all events
            // that we see.  since the next step below is to do a VM.load,
            // we want to make sure that any changes to the VM (which
            // will be seen as vminfod events) are recorded from the time
            // we call VM.load below, to the time we start watching for
            // expected changes to eliminate any possibily of a race
            log.debug('creating vminfod event stream');
            vs = new vminfod.VminfodEventStream({
            ...
            vs.on('ready', function (ready_ev) {
                ...
                vs.on('readable', function () {
                    var ev;
                    while ((ev = vs.read()) !== null) {
                        vminfod_events.push(ev);
                    }
                });

                ready_ev.vms.forEach(function (vm) {
                    if (vm.uuid === uuid) {
                        log.debug('found vm uuid (%s) in vminfod "ack" packet',
                            uuid);
                        vmobj = vm;
                    }
                });
            ...
            // loop each key from the original payload, and construct
            // an array of changes block for on vminfod
            keys.forEach(function (key) {
                if (key === 'zonename')
                    return;
                ...
                /* JSSTYLED */
                re = /^set_(customer_metadata|internal_metadata|tags|routes)$/;
                if ((m = key.match(re))) {
                    label = m[1];
                    existing = vmobj[label] || {};

                    assert.object(value, 'value');
                    Object.keys(value).forEach(function (k) {
                        var v = value[k];
                        change = {
                            path: util.format('%s.%s',
                                label, k),
                            to: v
                        };
                ...
                /* JSSTYLED */
                re = /^remove_(customer_metadata|internal_metadata|tags|routes)$/;
                if ((m = key.match(re))) {
                    label = m[1];
                    existing = vmobj[label] || {};
                ...
                if ((m = key.match(/^update_(.*)$/))) {
                    ...
                    log.debug({arr: vmobj[label], map: map},
                        'array index map built for "%s"', key);

                    if (!value)
                        value = [];
                    value.forEach(function (thing) {
                        var index = map[thing[identifierKey]];
                        assert.number(index, 'index for '
                            + thing[identifierKey]);
                        ...
                        Object.keys(thing).forEach(function (_key) {
                            ...
                            var defaultPath = util.format('%s.*.%s',
                                label, _key);
                            var lookingFor = util.format('%s.%d.%s',
                                label, index, _key);
                            ...
                            if (vmobj[label][index][_key] !== thing[_key]) {
                                changes.push(localChange);
                            }
                        });
                    });
                ...
                if (o && o.hasOwnProperty('pr_default')) {
                    ...
                    // if the value is being set to the default value for
                    // the property, this is equivalent to value removal
                    if (def === value) {
                        delete change.to;
                        change.action = 'removed';
                    }
                ...
                // if the current value matches the desired value, we skip
                // it as no work is needed to be done
                ...
                if (change.action === 'removed') {
                    ...
                    // sometimes changing a value to 0 is equivalent to removing
                    // that value (ex: "cpu_cap").  in this case, we push 2
                    // changes so only 1 of them needs to be met
                    change = [change];
                    change.push({
                        path: change[0].path,
                        action: 'removed'
            ...
            vasync.parallel({
                funcs: [
                    function (cb2) {
                        ...
                        log.debug({obj: obj, changes: changes},
                            'watching for vminfod changes');
                        vs.removeAllListeners('readable');
                        vs.watchForChanges(obj, changes, opts,
                            function (err) {
                            vs = null;
                            if (err) {
...
function halt(uuid, log, callback)
{
    ...
    vasync.pipeline({funcs: [
        function (_, cb) {
            ...
            // watch for the vminfod event and simultaneously execute the
            // commands to halt the zone.

            var waitonvminfod = true;
            ...
            log.debug({obj: obj}, 'watching for vminfod down event');
            vs.watchForEvent(obj, opts, function (e, _r) {
                if (!waitonvminfod) {
                    // this boolean is set to false if we don't need to wait on
                    // vminfod.  this is the case when the zoneadm command
                    // fails or when a vm is already down.
                    log.debug('skipping vminfod down event');
                    return;
                }
                ...
                log.debug('got vminfod down event');
                cb();
            });
                ...
                if (msg.match(/zone is already halted$/)) {
                    ...
                    VM.unsetTransition(vmobj, {log: log}, function () {
                        ...
                        new_err = new Error(sprintf(
                            'VM %s is already not \'running\' (currently: %s)',
