                ...
                fs.exists(filename, function (exists) {
                    if (exists) {
                        ...
                        log.warn('Marking VM ' + vmobj.uuid + ' as a "failure"'
                            + ' because we still have /var/svc/provisioning '
                            + 'after rename');
                        VM.markVMFailure(vmobj, {log: log}, function (err) {
                            if (err) {
                                log.warn(err, 'markVMFailure(): '
                                    + err.message);
                            }
                            done();
                    ...
                    // So long as /var/svc/provisioning is gone, we don't care
                    // what replaced it.  Success or failure of user script
                    // doesn't matter for the state, it's provisioned now.
                    // Caller should now clear the transition.
                    done();
                    cb();
                    return;
                });
