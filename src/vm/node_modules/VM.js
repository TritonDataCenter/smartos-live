        ...
        function (cb) {
            ...
            normalizePayload(payload, vmobj, log, function (e) {
                ...
                keys.forEach(function (key) {
                    ...
                    // disks.*.value
                    if (['update_disks', 'update_nics'].indexOf(key) > -1) {
                        value.forEach(function (disk) {
                            ...
                            Object.keys(disk).forEach(function (_key) {
                                if (_key === 'path')
                                    return;

                                var prefix = key.replace(/^update_/, '');
                                var defaultPath = util.format('%s.*.%s',
                                    prefix, _key);
                                var lookingFor = util.format('%s.%d.%s',
                                    prefix, diskIndex, _key);

                                var localChange = {
                                    path: lookingFor
                                };

                                localChange.to = disk[_key];
