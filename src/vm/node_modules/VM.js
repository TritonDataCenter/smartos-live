...
function reprovisionBhyve(uuid, vmobj, payload, log, cb)
{
    ...
    vasync.waterfall([
        ...
        function destroyOldDisk0(next) {
            ...
            zfs(['destroy', '-f', disk0_ds], log, function (err, fds) {
                if (err) {
                    log.error({
                        err: err,
                        stdout: fds.stdout,
        ...
        function setQuotaBhyveAfterReprovision(next) {
            // Recalculate quotas after creating new disk0
            var dataset = vmobj.zfs_filesystem;
            var quota = vmobj.quota || DEFAULT_BHYVE_QUOTA;
            var flexible_disk_size = vmobj.flexible_disk_size;

            log.info('Recalculating quotas for dataset %s', dataset);
            setQuotaBhyve({
                dataset: dataset,
                log: log,
                quota: quota,
