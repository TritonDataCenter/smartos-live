        ...
        function (cb) {
            ...
            vs.once('ready', function (ready_ev) {
                try {
                    ...
                    if (!vmobj.zpool) {
                        log.warn('failed to load \"zpool\" field, retrying via manual load');

                        VM.load(uuid, {log: log, loadManually: true}, function (err, obj) {
                            if (err) {
                                cb(err);
                                return;
                            }
            ...
            // unmounting using the mountpoint fails often with: "cannot unmount '/zones/<vm_uuid>/cores': not a mountpoint"
            // so use the dataset name instead of the mountpoint which seems to solve the issue
            zfs(['unmount', vmobj.zpool + '/cores/' + uuid], log,
                function (err, fds) {
            ...
            var retry_count = 0;    // number of retries so far
            var retry_max = 10;     // max retries
                ...
                setTimeout(function _doRetryBusy() {
                    zfs(['mount', vmobj.zfs_filesystem], log,
                        function (err, fds) {
                            ...
                            '(retry + ' + (retry_count + 1) + ') mount results');

                            retry_count++;
                            
                            if (err && retry_count < retry_max) {
                                _retryBusy();
                            }
                            else {
                                cb(err);
        ...
        function _loadVM(cb) {
            ...
            VM.load(uuid, {log: log, fields: load_fields}, function (err, obj) {
                ...
                if ((obj.state !== 'stopped'
                    ...
                    if (obj.state === 'provisioning' && obj.zone_state !== 'installed') {
                        var retry_delay = 1;    // second(s) between retries
                        var retry_count = 0;    // number of retries so far
                        var retry_max = 10;     // max retries

                        function _checkZoneState() {
                            log.warn('zone_state was not "installed", reloading VM object');

                            setTimeout(function _doCheckZoneState() {
                                VM.load(uuid, {log: log, fields: ['state','zone_state']}, function (retryErr, retryObj) {
                                    retry_count++;

                                    if (retryErr) {
                                        log.error(retryErr, "failed to reload VM object (retry " + retry_count + ")");
                                        cb(retryErr);
                                        return;
                                    }

                                    if (retryObj.zone_state === 'installed') {
                                        vmobj = obj;
                                        cb();
                                        return;
                                    }
