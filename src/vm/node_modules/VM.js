            ...
            vasync.parallel({
                funcs: [
                    function (cb2) {
                        ...
                        if (payload.hasOwnProperty('zfs_data_compression'))
                            obj.vm.zfs_data_compression =
                                payload.zfs_data_compression;
                        if (payload.hasOwnProperty('zfs_data_recsize'))
                            obj.vm.zfs_data_recsize = payload.zfs_data_recsize;
                        var opts = {
                            timeout: VMINFOD_TIMEOUT,
                            log: log,
                            catchErrors: true
        ...
        function (cb) {
            ...
            normalizePayload(payload, vmobj, log, function (e) {
                ...
                keys.forEach(function (key) {
                    ...
                    // update disks requires converting the path to a generic
                    // disks.*.value
                    if (key === 'update_disks') {
                        value.forEach(function (disk) {
                            var o = {};
                            vmobj.disks.forEach(function (d, i) {
                                o[d.path] = i;
                            });
                            var diskIndex = o[disk.path];
                            var diskChanges = disk;
                            Object.keys(diskChanges).forEach(function (key) {
                                if (key === 'path')
                                    return;

                                var defaultCheck = util.format("%s.*.%s", 'disks', key);
                                var lookingFor = util.format("%s.%d.%s", 'disks', diskIndex, key);

                                var localChange = {
                                    path: lookingFor
                                };

                                localChange.to = disk[key];

                                // if the current disk value matches the change, we
                                // we can bail early on building the changes
                                if (PAYLOAD_PROPERTIES[defaultCheck].hasOwnProperty('pr_default')) {
                                    var def = PAYLOAD_PROPERTIES[defaultCheck].pr_default;
                                ...
                                if (vmobj.disks[diskIndex][key] !== disk[key]) {
