...
function loadDatasetInfo(m, callback)
{
    ...
    args = ['list', '-H', '-p', '-o', 'name,quota,mountpoint,type'];
        ...
        loadMetadata(vmobj, function (err, metadata, tags) {
            ...
            if (data.dsinfo.hasOwnProperty(vmobj.zonepath)) {
                ...
                VM.log('DEBUG', 'found quota "' + vmobj.quota + '" for ' +
                    vmobj.uuid);
                vmobj.zone_root_dataset = data.dsinfo[vmobj.zonepath].dataset;
                VM.log('DEBUG', 'found dataset "' + vmobj.zone_root_dataset +
                    '" for ' + vmobj.uuid);
            } else {
                VM.log('DEBUG', 'no dsinfo for ' + vmobj.uuid);
            }

            if (obj.hasOwnProperty('zone_root_dataset')) {
                obj.zfs_storage_pool_name = obj.zone_root_dataset.split('/')[0];
            }
...
function addDelegatedDataset(payload, callback)
{
    ...
    if (payload.delegate_dataset) {
        dataset = payload.zone_root_dataset + '/data';
...
function createZone(payload, callback)
{
    ...
    payload.zone_root_dataset = payload.zfs_storage_pool_name + '/' +
        payload.zonename;
    payload.zonepath = '/' + payload.zone_root_dataset;
    ...
    async.series([
        function (cb) {
            ...
            if (payload.brand === 'joyent' && payload.autoboot && !payload.nowait) {
    ...
    if (extra.hasOwnProperty('disks')) {
        for (disk in extra.disks) {
            if (extra.disks.hasOwnProperty(disk)) {
                ...
                // file that's been dropped in to the zoneroot
                found = false;
                for (d in vmobj.disks) {
                ...
                if (!found && path.existsSync(vmobj.zoneroot + '/' +
                    disk.path)) {
...
function applyUpdates(oldobj, newobj, payload, callback)
{
    ...
    async.series([
        function (cb) {
            ...
            if (payload.hasOwnProperty('quota')) {
                setQuota(newobj.zone_root_dataset, payload.quota, cb);
            } else {
                cb();
            }
        }, function (cb) {
