...
function OpenOnErrorFileStream(filename) {
    ...
    this.newStream = function (callback) {
        ...
        fs.open(self.path, 'a', mode, function (err, fd) {
            ...
            // We can call fs.watch() here safely because we know the file
            // was just opened. See OS-2904 for some more details.
...
exports.receive = function (target, options, callback)
{
    ...
    receiveStdinChunk('JSON', log, function (error, result) {
        ...
        if (result.brand === 'lx') {
            callback(new Error('receive of LX VMs is not yet supported'));
            return;
        }
...
exports.create = function (payload, options, callback)
{
    ...
    if (payload.brand === 'lx') {
        callback(new Error('creation of LX VMs is not yet supported'));
        return;
    }
...
exports.rollback_snapshot = function (uuid, snapname, options, callback)
{
    ...
    VM.load(uuid, {fields: load_fields, log: log}, function (err, vmobj) {
        ...
        var snap;
        var snapshot_list = [];
        var stopped = false; // Whether *we* stopped it (if so we'll start)
        ...
        function stopZone(cb) {
            VM.stop(vmobj.uuid, {'force': true, log: log}, function (e) {
                if (!e) {
                    stopped = true;
                }
                cb(e);
        ...
        vmsnap.rollbackSnapshot(vmobj, snapname, rollback_opts,
            function _rolledBackCB(err) {
                if (err) {
                    // XXX rollback failed, this is bad.
                    callback(err);
                    return;
                }

                // completed rollback, now start again if we stopped and we
                // want to start.
...
exports.delete_snapshot = function (uuid, snapname, options, callback)
{
    ...
    VM.load(uuid, {fields: load_fields, log: log}, function (err, vmobj) {
        ...
        vmsnap.deleteSnapshot(vmobj, snapname, {log: log}, callback);
    });
};
...
exports.create_snapshot = function (uuid, snapname, options, callback)
{
    ...
    VM.load(uuid, {fields: load_fields, log: log}, function (err, vmobj) {
        ...
        vmsnap.createSnapshot(vmobj, snapname, {log: log}, callback);
    });
};
