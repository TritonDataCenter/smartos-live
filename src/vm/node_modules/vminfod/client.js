 ...
 *     }
 * });
XXX should doc usage of the `.stop()` method to cleanly stop.
 *
 ...
 *     if (err) {
XXX Any spec on this `err`? Known name or info if a timeout?
XXX Does calling `cancelFn` count as "stream ended prematurely"? Or only if `cancelFn(err)` err arg is passed in?
...
XXX does order of changes matter?
 * var changes = [
 *    {
 *        path: ['quota'],
 *        action: 'changed',
