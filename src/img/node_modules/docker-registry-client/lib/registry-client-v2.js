...
/*
 * Retrieves a manifest digest from a MEDIATYPE_MANIFEST_LIST_V2 or a
 * MEDIATYPE_OCI_MANIFEST_LIST_V1 payload.
 * The retrieved digest needs to be used by client.getManifest to
 * retrieve the actual manifest object.
 *
 * client.getManifestDgst({ref: <tag>}, function (err, manifestDgst, res) {
 *   });
 * @param ref: {Object}
 * @param cb {Function} `function (err, manifestDgst, res)`
 */
RegistryClientV2.prototype.getManifestDgst =function getManifestDgst(opts, cb) {
    var self = this;
    assert.object(opts, 'opts');
    assert.string(opts.ref, 'opts.ref');
    assert.optionalBool(opts.acceptManifestLists, 'opts.acceptManifestLists');
            ...
            self._api.get({
                ...
                if (manifest.mediaType === MEDIATYPE_MANIFEST_LIST_V2
                    ...
                    for (var i = 0; i < manifest.manifests.length;  i++) {
                        var platform = manifest.manifests[i].platform;
                        if (platform &&
                    ...
                    if(!manifestDigest) {
                       cb(new restifyErrors.InvalidContentError(
                        'No linux/amd64 platform found in Manifest'));
                       return;
                    }
                }
