  ...
  parseData = function() {
    ...
    for (var i=0; i<lines.length; i++) {
      ...
      if (old == name) {
        ...
        old = name;
        if (image.name) images.push(_formatRecord(image));
        image = {name: name};
        image[prop] = val;
      }
    }
...
list = function(options, callback) {
  ...
  zfs.stderr.on('data', function(chunk) {
    callback(newError(ERR_LIST, "error listing images"), null);
  });  
...
rename = function(name, newname, callback) {
  ...
  zfs.stderr.on('data', function(chunk) {
    if (callback) callback(newError(ERR_RENAME, "error renaming image"));
  });
