...
function lookupConflicts(macs, ips, callback) {
    ...
    if (macs.length === 0 && ips.length === 0) {
        VM.logger('DEBUG', 'returning from conflict check (nothing to check)');
    ...
    if (vmobj.hasOwnProperty('nics')) {
        for (n in vmobj.nics) {
            if (vmobj.nics[n].hasOwnProperty('interface')) {
                idx = vmobj.nics[n].interface.match(/^net(\d+)$/)[1];
                used_nic_indexes.push(Number(idx));
            }
        }
    }
    ...
    VM.load(uuid, function (err, obj) {
        ...
        if (obj.hasOwnProperty('disks')) {
            for (n in vmobj.disks) {
                idx = vmobj.disks[n].path.match(/^.*-disk(\d+)$/)[1];
                used_disk_indexes.push(Number(idx));
            }
        }
