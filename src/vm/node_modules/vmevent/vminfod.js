...
function vmDatasets()
{
    if (vm_data_tmp !== undefined) {
        return ([vm_data, vm_data_tmp]);
    } else {
        return ([vm_data]);
    }
}
...
function handleFsEvent(obj)
{
    ...
    log.debug('FS_EVENT: ' + JSON.stringify(obj));
    ...
    event_queue.enqueue(function (callback) {
        if (zonexml_re.test(obj.pathname)) {
            ...
            if (!vmobjs.hasOwnProperty(zonename)) {
                log.debug('vmobj doesn\'t exist for ' + zonename);
                callback();
                return;
            }

            if (obj.type == 'delete') {
                log.debug(obj.pathname + ' deleted');
                // the show is over! teardown!
                async.series([
                    // unregister events
                    function (cb) {
                        stopVmWatchers(zonename, cb);
                    },
                    // delete vmobj
                    function (cb) {
                        delete vmobjs[zonename];
                        cb();
                    },
                    // clean vm_data
                    function (cb) {
                    ...
                    log.info('stopped watching ' + zonename);
                    callback();
                });
            } else {
                log.debug(obj.pathname + ' modified');
                ...
                refreshVmobj(zonename, callback);
            }
...
function handleZoneEvent(obj)
{
    ...
    log.debug('ZONE_EVENT: ' + JSON.stringify(obj));
    ...
    event_queue.enqueue(function (cb) {
        log.debug('handling zone event for ' + zonename);

        if (!vmobjs.hasOwnProperty(zonename)) {
            log.debug('preparing to register and watch ' + zonename);
            // a new vm!
            ...
            vmload.getVmobj(zonename, opts, function (error, vmobj) {
                if (error) {
                    ...
                    startVmWatchers(zonename, function (err) {
                        log.info('started watching ' + zonename);
...
function handleZpoolEvent(obj)
{
    ...
    log.debug('ZPOOL_EVENT: ' + JSON.stringify(obj));
    ...
    function fetch(cb) {
        var opts = {log: log, dataset: name};
        getDatasets(opts, function (err, results) {
            if (err) {
                log.warn('failed to fetch datasets');
            }
            cb(err, results);
        });
    }
    ...
    event_queue.enqueue(function (cb) {
        log.debug('handling zfs event for ' + name);
        ...
        if (uuid_re.test(name)) {
            ...
            log.debug('refreshing vmobj ' + uuid + ' after zfs event');
 ...
 */
function refreshVmobj(zonename, callback)
{
    // short-circuit if this vmobj doesn't exist
    if (!vmobjs.hasOwnProperty(zonename)) {
        log.debug('cannot refresh vmobj, doesn\'t exist');
        callback();
        return;
    }
    ...
    async.waterfall([
        ...
        function (cb) {
            log.debug('refreshing zoneData for ' + zonename);
            var opts = {log: log, cache: vm_data};
            vmload.getZoneData(zonename, opts, function (err, cache) {
                ...
                if (err) {
                    log.error('unable to refresh vmobj for ' + zonename
        ...
        // request a new vmobj
        function (cb) {
            log.debug('requesting new vmobj for ' + zonename);
            var opts = {log: log, cache: vm_data};
    ...
    async.series([
        function (cb) {
            ...
            fsw.once('ready', function (obj) {
                log.debug('filesystem watcher ready');
                cb();
            });
            ...
            zonew.once('ready', function (obj) {
                log.debug('zone event watcher ready');
                cb();
            });
        ...
        log.info('global watchers ready');
        callback();
    });
}
        ...
        async.waterfall([
            ...
            function (cb) {
                getZoneRecords(null, {log: log}, function (err, records) {
                    if (err) {
                        log.debug('failed to load zone records: '
                            + err.message);
                    }
                    cb(err, records);
                });
            ...
            if (err) {
                log.error('failed to start vm watchers');
            }
            callback(err);
        });
    } else {
        addVMFsWatches(vmobjs[zonename].zonename);
        callback();
    }
}
 ...
 *   for all vms.
 */
function stopVmWatchers(zonename, callback)
{
    if (arguments.length === 1) {
        callback = zonename;
        zonename = undefined;
    ...
    if (zonename === undefined) {
        for (zonename in vmobjs) {
            removeVMFsWatches(zonename);
        }
    } else {
        removeVMFsWatches(vmobjs[zonename].zonename);
    }
...
{
    log.trace('adding vm fs watchers for ' + zonename);
...
{
    log.trace('removing vm fs watchers for ' + zonename);

    function unwatch(pathname) {
        fsw.unwatch(pathname, function (err) {
            if (err) {
...
function setVmData(callback)
{
    loadVmData(function (err, cache) {
        if (err) {
            log.error('failed to set initial vm_data');
        } else {
            log.debug('setting initial vm_data: ' + cache);
            vm_data = cache;
        }
        callback(err);
    });
...
function loadVmData(callback)
{
    ...
    vmload.getZoneData(null, opts, function (err, cache) {
        if (err) {
            log.debug('failed vmload.getZoneData: ' + err.message);
        }
        callback(err, cache);
    });
}
...
function setVmobjs(callback)
{
    vmload.getVmobjs({cache: vm_data, log: log}, function (err, results) {
        if (err) {
            log.debug('failed vmload.getVmobjs: ' + err.message);
            log.error('failed to set vmobjs');
        } else {
            vmobjs = {};
...
function startHTTPHandler(callback)
{
    ...
    for (ip in ips) {
        ip = ips[ip];
        log.debug('LISTENING ON ' + ip + ':' + VMINFOD_PORT);
        http.createServer(handler).listen(VMINFOD_PORT, ip);
    }
}
    ...
    async.series([
        ...
        function (cb) {
            loadVmData(function (err, results) {
                if (err) {
                    log.error('failed to fetch new vm_data');
                } else {
                    vm_data_tmp = results;
                }
                cb(err);
        ...
        function (cb) {
            ...
            log.debug('fast-forwarding the queue to ' + now);
            event_queue.fastForward(now, cb);
        },
        ...
        // resume the queue
        function (cb) {
            log.debug('resuming the event queue');
            vm_data_tmp = undefined;
            event_queue.resume();
            cb();
        }
    ], callback);
...
function main()
{
    async.series([
        ...
        if (err) {
            log.error('failed to complete boot sequence');
            process.exit(1);
        } else {
