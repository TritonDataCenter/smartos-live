        ...
        function (cb) {
            ...
            vs.once('ready', function (ready_ev) {
                try {
                    ...
                    if (!vmobj.zpool) {
                        log.warn('failed to load \"zpool\" field, retrying '
                            + 'via manual load');
            ...
            // unmounting using the mountpoint occasionally fails with:
            //      "cannot unmount '/zones/<vm_uuid>/cores': not a mountpoint"
            // so use the dataset name instead of the mountpoint which seems to
            // solve the issue
            zfs(['unmount', vmobj.zpool + '/cores/' + uuid], log,
                ...
                if (err) {
                    if (trim(fds.stderr)
                        .match(/(not a mountpoint|not currently mounted)$/)) {
            ...
            VM.load(uuid, {log: log, fields: load_fields}, function (err, obj) {
                ...
                if ((obj.state !== 'stopped'
                    ...
                    if (obj.state === 'provisioning'
                        ...
                        var retry_count = 0;    // number of retries so far
                        var retry_max = 10;     // max retries
                        var zone_state = obj.zone_state;

                        function _checkZoneState() {
                            log.warn('zone_state is "' + zone_state +
                            '" rather than "installed", reloading VM object');

                            setTimeout(function _doCheckZoneState() {
                                VM.load(uuid, {
                                    log: log,
                                    fields: load_fields
                                }, function (retryErr, retryObj) {
                                    retry_count++;

                                    if (retryErr) {
                                        log.error(retryErr,
                                            'failed to reload VM object (retry '
                                            + retry_count + ')');
                                        cb(retryErr);
                                        return;
                                    }

                                    zone_state = retryObj.zone_state;

                                    if (zone_state === 'installed') {
                                        vmobj = retryObj;
                                        cb();
                                        return;
                                    }
