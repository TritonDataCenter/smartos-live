...
var IMG = require('/usr/img/lib/IMG');
...
function validateImage(image, log, callback)
{
    ...
    if (process.env.EXPERIMENTAL_VMJS_TRACING) {
        tracers_obj = traceUntilCallback('validate-image', log, callback);
        callback = tracers_obj.callback;
        log = tracers_obj.log;
    }
    ...
    IMG.quickGetImage(opts, function (err, data) {
        var e;
        ...
        log.info('image ' + image.uuid + ' manifest found in imgadm');
...
exports.load = function (uuid, options, callback)
{
    ...
    if (options.hasOwnProperty('nocache')) {
        load_opts.nocache = options.nocache;
    }
...
exports.waitForZoneState = function (payload, state, options, callback)
{
    ...
    watcher = watchZoneTransitions(payload.zonename, log);

    watcher.on('statechange', function (_from, to) {
        if (to === state) {
            // found the state we're looking for, success!
            log.debug('saw zone go to ' + state + ' ('
...
function watchZoneTransitions(zonename, log)
{
    var emitter = new EventEmitter();

    emitter.zonename = zonename;
    ...
    vminfo.isOnline(function (online) {
        if (online) {
            watchVminfoTransitions(emitter, log);
        } else {
            watchZoneeventTransitions(emitter, log);
        }
    });

    emitter.cleanup = function () {
        // watcher specific function defined by watcher (vminfo or zonevent)
        emitter.cleanupWatcher();
...
function watchVminfoTransitions(emitter, log)
{
    var zonename = emitter.zonename;
    ...
    var request = http.get(opts, function (res) {
        ...
        res.on('data', function (data) {
            ...
            while (chunks.length > 1) {
                ...
                if (task.zonename !== zonename) {
                    continue;
                }

                switch (task.type) {
                case 'create':
                    emitter.emit('statechange', null, task.vm.zone_state);
                    break;
                case 'modify':
                    task.changes.forEach(function (change) {
                        if (change.path === 'zone_state') {
                            emitter.emit('statechange', change.from, change.to);
                        }
                    });
                    break;
                case 'delete':
                    emitter.emit('statechange', null, 'deleted');
                    break;
                default:
                    break;
                }
        ...
        res.on('close', function () {
            log.error('vminfod events closed prematurely');
            emitter.emit('error',
                new Error('Events channel closed prematurely'));
            emitter.cleanup();
        });
    }).on('error', function (err) {
        log.error('vminfod request failed: ' + err.message);
        emitter.emit('error',
            new Error('Unable to subscribe to vminfod events'));
        emitter.cleanup();
    });

    emitter.cleanupWatcher = function () {
        request.end();
        request.abort();
    };
}

function watchZoneeventTransitions(emitter, log)
{
    var zonename = emitter.zonename;
    var buffer = '';
    var chunks;
            ...
            if (obj === prev_msg) {
                ...
                if (obj.zonename == zonename) {
                    emitter.emit('statechange',
                        translateState(obj.oldstate),
                        translateState(obj.newstate));
                }
            }
        }
        buffer = chunks.pop();
    ...
    watcher.on('exit', function (code) {
        ...
        emitter.emit('error', new Error('zoneevent watcher exited '
...
function deleteZone(uuid, log, callback)
{
    ...
    async.series([
        ...
        // TODO: replace these next two with VM.stop(..{force: true} ?
            ...
            VM.load(uuid, opts, function (err, obj) {
...
function doReboot(vmobj, options, callback)
{
    ...
    async.series([function (cb) {
        if (!reboot_async) {
            watcherobj = watchZoneTransitions(vmobj.zonename, log);

            watcherobj.on('statechange', function (from, to) {
            ...
            watcherobj.on('error', function (err) {
