        ...
        function _loadVM(cb) {
            ...
            if (['BHYVE', 'KVM'].indexOf(vm_type) !== -1) {
                ...
                VM.load(uuid, {
                    ...
                    // Determine if CloudInit filesystem needs to be recreated
                    // for NoCloud VMs before starting the VM
                    if (!im || !noCloudPath
                        || im.cloudinit_datasource !== 'nocloud') {
                    ...
                    log.debug({path: noCloudPath},
                        'Generating CloudInit filesystem for nocloud VM');
                    ...
                    cloudinit.nocloud.createVolume(log, cloudInitPayload,
                        im.cloudinit_nocloud_path,
                        function _createCloudInitNoCloudVolume(ciErr) {
                        if (ciErr) {
                            log.error({err: ciErr},
                                'Failed to generate CloudInit filesystem');
                            cb(ciErr);
                            return;
                        }
                        log.debug('Generated CloudInit filesystem');
                        startVM(obj, extra, log, cb);
                    });
