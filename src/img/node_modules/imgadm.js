...
ZPOOL_NAME = 'zones';
IMGAPI_URL  = 'https://datasets.joyent.com/datasets/'; // need trailing slash
DB_CACHE   = '/var/db/imgadm/imgcache.json';
DB_SOURCES = '/var/db/imgadm/sources.list';
...
imageExists = function(uuid, callback) {
...
var cacheList = function (callback) {
  ...
  if (!cache) {
    return callback("Cache file couldn't be loaded. Try 'imgadm update' first", null);
  }
...
var onImport = function(manifest, callback) {
  ...
  zfs.rename(volname + '-partial', volname, function(err) {
    ...
    zfs.setProp(volname, "imgadm:uuid", uuid, function(err) {
      if (err) return callback(err, uuid);
      zfs.setProp(volname, "imgadm:urn", manifest.urn, function(err) {
        if (err) return callback(err, uuid);
...
var importRemote = function(uuid, callback) {
  ...
  show(uuid, function(error, manifest) {
    ...
    var inflater, imgfile, _url;

    imageExists(uuid, function(exists) {
      if (exists)
        return callback("image already installed");
...
destroyImage = function(name, callback) {
  assert.ok(name);
  assert.ok(callback);
  ...
  dump(name, function(err, dump) {
    ...
    if (numClones > 0) {
      ...
      return callback("image has children. cannot destroy");
    }
    else {
