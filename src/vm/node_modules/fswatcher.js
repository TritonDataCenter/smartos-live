    ...
    self.res_queue = vasync.queue(function stdoutQueue(line, cb) {
        ...
        if (delta > MESSAGE_DELAY_LOG_THRESHOLD) {
            self.log.warn({deltaMs: delta, avg: self.messageDelays()},
                'message took greater than %dms to process', delta);
        }
...
/*
 * Calculate the averages for the message delay buffer (in milliseconds)
 */
FsWatcher.prototype.messageDelays = function messageDelays() {
    ...
    var i;
    var o = {};
    var msRecords = self.message_delay_buffer.records;
    var sum;
     ...
     */
    for (i = 1; i <= msRecords.length; i *= 10) {
        sum = msRecords.slice(0, i).reduce(function sumRecords(a, b) {
            return a + b;
        }, 0);
        avg = sum / i;
        o[i] = avg;
