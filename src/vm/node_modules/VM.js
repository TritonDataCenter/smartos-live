...
function checkPayloadProperties(payload, vmobj, callback)
{
    ...
    if (!vmobj) {
        ...
        if (vm_type === 'kvm' && payload.hasOwnProperty('available_MiB')) {
    ...
    if (vm_type === 'kvm' && payload.hasOwnProperty('add_disks')) {
        for (disk in payload.add_disks) {
            if (payload.add_disks.hasOwnProperty(disk)) {
                zvol = payload.add_disks[disk];
    ...
    if (vm_type === 'kvm' && payload.hasOwnProperty('update_disks')) {
        ...
        if (payload.hasOwnProperty(field)) {
            for (nic in payload[field]) {
                if (payload[field].hasOwnProperty(nic)) {
                    ...
                    if (vm_type === 'kvm' && (!n.hasOwnProperty('model')
                        || n.model === 'undefined')) {
...
function normalizePayload(payload, vmobj, callback)
{
    ...
    // if there's a zfs_root_* and no zfs_data_*, normally the properties would
    // fall through, we don't want that.
    if (payload.hasOwnProperty('zfs_root_compression')
