 ...
 * };
 * var opts = {
 *     ident: "booting zone", // human readable string for debugging, required
...
VminfodEventStream.prototype.watchForEvent =
    ...
    assert.object(opts, 'opts');
    assert.string(opts.ident, 'opts.ident');
    ...
    var log = self.vs_logger.child({
        ident: f('vminfod watchForEvent "%s"', ident)
    });

    if (opts.timeout) {
        timeout = setTimeout(function vminfodEventStreamWatchForEventTimeout() {
            var msg = f('vminfod watchForEvent "%s" "%s" timeout exceeded',
                self.vs_name, ident);

            log.error({obj: obj, events: events}, '%s - unmatched events', msg);
    ...
    function processEvent(ev) {
        ...
        log.debug({obj: obj}, 'vminfod watchForEvent matched event');

        done(null, ev);
    }

    function onerror(e) {
        log.error({err: e}, 'vminfod watchForEvent stream error');
        done(e);
    }
    if (opts.catchErrors) {
    ...
    function cancelFn(err, o) {
        if (isDone) {
            log.warn('vminfod watchForEvent cancelled after done');
            return;
        }

        log.debug('vminfod watchForEvent cancelled');
        done(err, o);
    }
...
VminfodEventStream.prototype.watchForChanges =
    ...
    var ident = opts.ident;
    var log = self.vs_logger.child({
        ident: f('vminfod watchForChanges "%s"', ident)
    });
    ...
    if (total === 0) {
        log.warn('vminfod watchForChanges called with empty changes array');
    ...
    if (opts.timeout) {
        timeout = setTimeout(
            ...
            var msg = f('vminfod watchForChanges "%s" "%s" timeout exceeded',
                self.vs_name, ident);

            log.error({changes: changes, events: events},
    ...
    log.trace({changes: changes}, 'beginning watch for vminfod changes');
    ...
    function processEvent(ev) {
        ...
        log.trace({ev: ev}, 'inspecting possible match from vminfod');
            ...
            changes = changes.filter(function filterChanges(wantedArr) {
                ...
                for (var i = 0; i < wantedArr.length; i++) {
                    ...
                    if (match) {
                         ...
                         */
                        hasMatched = true;
                        log.trace({changeSeen: changeSeen,
        ...
        if (changes.length === 0) {
            ...
            log.debug('vminfod watchForChanges all changes matched');
            done();
            return;
        }

        log.debug({remaining: changes.length, total: total},
    ...
    function cancelFn(err, o) {
        if (isDone) {
            log.warn('vminfod watchForChanges cancelled after done');
            return;
        }

        log.debug('vminfod watchForChanges cancelled');
        done(err, o);
    }
