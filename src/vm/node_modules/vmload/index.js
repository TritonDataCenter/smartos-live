        ...
        function (cb) {
            ...
            if (cache.zonexml_objects
                ...
                for (var key in cache.zonexml_objects[uuid]) {
                    vmobj[key] = deepClone(cache.zonexml_objects[uuid][key]);
                }
 ...
 *   [ A, B, C, D ]
 *
 * If a filter is provided, filter() will be called with:
 *
...
function getVmobjs(filter, options, callback)
{
    ...
    // shift arguments if filter was omitted
...
function getZoneData(uuid, options, callback)
{
    ...
    async.series([
        ...
        function (cb) {
            /*
             * If you pass in a cache, we'll only load those members that are
             * not already present. (each load* returns if cache exists)
             *
             * If a cache is not provided, we will attempt to load a warm cache
             * from the vminfod socket. If this is unsuccessful, we'll keep the
             * cache object empty {} to ensure the load* functions do the work.
             */
            if (options.hasOwnProperty('cache')) {
                cache = options.cache;
                cb();
            ...
            // if someone explicitly wants to force the load*
            if (options.hasOwnProperty('nocache') && options.nocache === true) {
                cb();
                return;
            }

            if (process.env.hasOwnProperty('NOCACHE')) {
                cb();
                return;
            }

            var opts = {host: '127.0.0.1', port: 9090, path: '/data'};
                ...
                res.on('end', function () {
                    if (res.statusCode === 200) {
                        try {
                            ...
                            log.trace('using cache object from vminfod');
                        } catch (e) {
                            cache = {};
                            log.debug('failed to parse body from vminfod: '
                                + e.message);
                        }
                        cb();
                    } else {
                        log.debug('vminfod response not usable: '
                            + res.statusCode);
                        cb();
                    }
                });
            }).on('error', function (err) {
                log.debug('vminfod request failed: ' + err.message);
                cb();
            });
        },
        // load the data
        function (cb) {
            ...
            async.eachSeries([
                ...
                loadJsonObjects,
                loadDatasetObjects
            ], function _loadWrapper(fn, cb2) {
                /*
                 * we expect each parameter to read from options and set their
                 * data into cache.<whatever> when they're requested to.
                 */
                fn(uuid, cache, options, cb2);
            }, function (err) {
                if (err) {
                    callback(err);
                    return;
...
function loadJsonObjects(uuid, cache, options, callback)
{
    ...
    if (cache.hasOwnProperty('json_objects')) {
        if (uuid === null) {
            ...
            callback();
            return;
        } else if (cache.json_objects.hasOwnProperty(uuid)) {
            log.trace('using existing cache.json_objects.uuid');
            callback();
            return;
        }
    }
    ...
    if (uuid !== null) {
        ...
        uuids = Object.keys(cache.zoneadm_objects);
    }

    start_time = (new Date).getTime();

    // (parallel)
    async.each(uuids, function (vm_uuid, cb) {
        ...
        if (uuid !== null) {
            ...
            if (json_objects.hasOwnProperty(uuid)) {
                cache.json_objects[uuid] = json_objects[uuid];
            }
        } else {
...
function loadLastExited(uuid, cache, options, callback)
{
    ...
    if (cache.hasOwnProperty('last_exited')) {
        if (uuid === null) {
            ...
            callback();
            return;
        } else if (cache.last_exited.hasOwnProperty(uuid)) {
            log.trace('using existing cache.last_exited.uuid');
            callback();
            return;
        }
    }
    ...
    async.each(uuids, function (vm_uuid, cb) {
        ...
        if (uuid !== null) {
            ...
            if (last_exited.hasOwnProperty(uuid)) {
                cache.last_exited[uuid] = last_exited[uuid];
            }
        } else {
...
function loadLastModified(uuid, cache, options, callback)
{
    ...
    if (cache.hasOwnProperty('last_modified')) {
        if (uuid === null) {
            ...
            callback();
            return;
        } else if (cache.last_modified.hasOwnProperty(uuid)) {
            log.trace('using existing cache.last_modified.uuid');
            callback();
            return;
        }
    }
    ...
    async.each(uuids, function (vm_uuid, cb) {
        ...
        if (uuid !== null) {
            ...
            if (last_modified.hasOwnProperty(uuid)) {
                cache.last_modified[uuid] = last_modified[uuid];
            }
        } else {
...
function loadZoneinfoObjects(uuid, cache, options, callback)
{
    ...
    if (cache.hasOwnProperty('zoneinfo_objects')) {
        if (uuid === null) {
            ...
            callback();
            return;
        } else if (cache.zoneinfo_objects.hasOwnProperty(uuid)) {
            log.trace('using existing cache.zoneinfo_objects.uuid');
            callback();
            return;
        }
    }
    ...
    getZoneinfo(uuid, options, function (err, results) {
        ...
        if (uuid !== null) {
            if (!cache.hasOwnProperty('zoneinfo_objects')) {
                cache.zoneinfo_objects = {};
            }
            if (results.hasOwnProperty(uuid)) {
                cache.zoneinfo_objects[uuid] = results[uuid];
            }
        } else {
...
function loadZoneadmObjects(uuid, cache, options, callback)
{
    ...
    if (cache.hasOwnProperty('zoneadm_objects')) {
        if (uuid === null) {
            ...
            callback();
            return;
        } else if (cache.zoneadm_objects.hasOwnProperty(uuid)) {
            log.trace('using existing cache.zoneadm_objects.uuid');
            callback();
            return;
        }
    }
    ...
    getZoneRecords(uuid, options, function (err, results) {
        ...
        if (uuid !== null) {
            ...
            if (results.hasOwnProperty(uuid)) {
                cache.zoneadm_objects[uuid] = results[uuid];
            }
        } else {
...
function loadZonexmlObjects(uuid, cache, options, callback)
{
    ...
    if (cache.hasOwnProperty('zonexml_objects')) {
        if (uuid === null) {
            ...
            callback();
            return;
        } else if (cache.zonexml_objects.hasOwnProperty(uuid)) {
            log.trace('using existing cache.zonexml_objects.uuid');
            callback();
            return;
        }
    }
    ...
    // cache.zoneadm_objects if uuid === null
    async.each(uuids,
        function (vm_uuid, cb) {
            var obj = cache.zoneadm_objects[vm_uuid];
            ...
            if (uuid !== null) {
                if (!cache.hasOwnProperty('zonexml_objects')) {
                    cache.zonexml_objects = {};
                }
                if (vmobjs.hasOwnProperty(uuid)) {
                    cache.zonexml_objects[uuid] = vmobjs[uuid];
                }
            } else {
