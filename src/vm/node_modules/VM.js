...
{
    var images = '/var/imgadm/images/';
    var manifest = images + image.zpool + '-' + image.uuid + '.json';

    fs.readFile(manifest, function (error, content) {
        var data;
        var e;
...
function watchZoneTransitions(zonename, log) {
    ...
    request = http.get(opts, function (res) {
...
function deleteZone(uuid, log, callback)
{
    ...
    async.series([
        ...
        // TODO: replace these next two with VM.stop(..{force: true} ?
            ...
            watcherobj = watchZoneTransitions(uuid, log);

            watcherobj.on('statechange', function (_from, to) {
                if (to === 'deleted') {
            ...
            ival = setInterval(function () {
                ...
                if (ticks <= 0) {
                    ...
                    cb(new Error('timed out waiting for zone to delete'));
                    return;
                }
            }, 100);
        }, function (cb) {
