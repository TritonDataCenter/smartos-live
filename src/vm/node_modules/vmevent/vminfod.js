...
function handleZoneEvent(obj)
{
    ...
    channel = zonename + '-zone-event';

    event_queue.enqueue(channel, function (cb) {
...
{
    var channel = uuid + '-zone-vmobj';

    // there are multiple event sources that drop into here. So we want
    // to serialize these requests behind a uuid.
    event_queue.enqueue(channel, function (cb) {
        async.waterfall([
