...
Object.keys(proptable.properties).forEach(function (prop) {
    ...
    if (property.watchers) {
        property.watchers.forEach(function (w) {
            var matches;

            matches = w.match(/^fswatcher:(.*)$/);
            if (matches) {
                if (!watches.fswatches[matches[1]]) {
                    watches.fswatches[matches[1]] = [prop];
                } else {
                    watches.fswatches[matches[1]].push(prop);
                }
            }
        });
    }
    ...
    if (property.zoneinfo) {
        ...
        if (property.zoneinfo_format) {
            zoneinfo[property.zoneinfo].format = property.zoneinfo_format;
        }
