...
VminfodClient.prototype.vm = function vm(zonename, cb) {
    ...
    var endpoint = '/vm/' + encodeURIComponent(zonename);
    return self.get(endpoint, function (err, vmobj) {
        if (err && err.code === 404) {
...
VminfodEventStream.prototype.start = function start(opts) {
     ...
     */
    self.vs_req.on('error', function (err) {
...
VminfodEventStream.prototype._transform =
    ...
    case 'ready':
        ...
        if (self.vs_parseReady) {
            ...
            parsed.forEach(function (vmobj) {
                vms[vmobj.zonename] = vmobj;
            });
            ev.vms = vms;
            self.vs_logger.debug({vms: parsed.length},
                'vminfod took %dms to parse "ready" event, %d vms total',
                Date.now() - now, parsed.length);
        }
 ...
 */
VminfodEventStream.prototype._abort = function _abort(e) {
    var self = this;
 ...
 */
VminfodEventStream.prototype._clear_ready_timeout =
...
function VminfodWatcher(opts) {
    ...
    self.vw_vs.once('ready', function (ready_ev) {
