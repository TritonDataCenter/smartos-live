    ...
    vmobj.disks.forEach(function (d) {
        if (d.hasOwnProperty('path')
            ...
            if (dsobj.hasOwnProperty('refreservation')) {
                ...
                d.refreservation = Math.ceil(dsobj.refreservation /
                    (1024 * 1024));
...
function addFreeSpace(vmobj, dataset_objects, options)
{
    ...
    // ZFS does not set the mountpoint attribute for volumes. If dataset is a
    // volume, we set mountpoint to '/dev/zvol/rdsk/' + name in
    // vmload-datasets.js
    ...
    // Round down to nearest integer to try to avoid user accidentally
    // trying to grow disk by more than space available
    vmobj.free_space = Math.floor(freeSpace);
}
...
function addSnapshots(vmobj, dataset_objects, options)
{
    ...
    zoneroot_snapshots.forEach(function (snap) {
        ...
        if (matches && matches[1]) {
            snap_size = snap.size / 1024 / 1024; // in MiB
