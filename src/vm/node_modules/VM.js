...
function cleanupMessyDataset(zonepath, callback)
{
    ...
    path.exists(zoneroot, function (exists) {
        if (exists) {
            command = 'rm -rf '
...
function installZone(payload, callback)
{
    ...
    async.series([
        function (cb) {
            ...
            // If zone_dataset_uuid is allowed, that's what's used to set the
            // root.  If not, dataset_uuid is for an image's uuid and not root.
            allowed = BRAND_OPTIONS[payload.brand].allowed_properties;
            if (allowed.hasOwnProperty('zone_dataset_uuid')) {
                if (payload.hasOwnProperty('zone_dataset_uuid')) {
                    args.push('-t', payload.zone_dataset_uuid, '-x',
                        'nodataset');
                }
            } else if (payload.hasOwnProperty('dataset_uuid')) {
                args.push('-t', payload.dataset_uuid, '-x', 'nodataset');
            }
...
function normalizePayload(payload, vmobj, callback)
{
    ...
    VM.validate(brand, action, payload, function (errors) {
        ...
        // When creating a VM with SPICE you need the zone_dataset_uuid, if you
        // don't pass that, we'll remove any SPICE options.
        if (action === 'create'
