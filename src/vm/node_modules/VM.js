...
function buildFilesystemZonecfg(vmobj, payload, options)
{
    ...
    for (filesystem in add) {
        ...
        // we'll support 'create' as a magic string to indicate that
        // this volume doesn't exist yet.
        if (filesystem.source === ':vmadm-create-filesystem:') {
        ...
        function (cb) {
            ...
            if (data_volumes.length === 0) {
                log.debug({uuid: vmobj.uuid}, 'VM has no local data volumes '
                    + 'no need to check other VMs for dependant filesystems');
                cb();
                return;
            }
            ...
            VM.lookup({}, {fields: [
                ...
                if (err) {
                    log.error({err: err, uuid: vmobj.uuid}, 'failed to lookup '
                        + 'potential dependant filesystems');
                    cb(err);
                    return;
                }
                ...
                log.error({uuid: vmobj.uuid, volume_users: volume_users},
                    'found VMs using dependant filesystems, cannot delete');
