...
Qmp.prototype.connect = function (socket, callback)
{
    ...
    qmp.stream.on('data', function (chunk) {
        ...
        while (chunks.length > 1) {
            ...
            if (result.hasOwnProperty('QMP')) {
                ...
                qmp.logger('INFO', 'QMP[' + result.id + '] result: ' + JSON.stringify(result));
                if (result.hasOwnProperty('id')) {
                    ...
                    log('WARNING: no id in result [', result, ']');
                }
    ...
    function on_connect(err) {
        if (!err) {
            qmp.command('qmp_capabilities', null, function (err) {
...
Qmp.prototype.command = function (command, args, callback)
{
    ...
    qmp.in_transit[cmd_id] = { 'packet': packet, 'sent': false, 'callback': callback };
