...
var VMInfo = module.exports = function (options) {
    ...
    function handleZoneEvent(obj) {
        ...
        // try to reduce duplicate events
        // if (self.vm_data.zoneadm_objects.hasOwnProperty(zonename)) {
        //     current_state = self.vm_data.zoneadm_objects[zonename].state;
        //     if (current_state == obj.newstate) {
        //         self.log.debug('discarding duplicate zone event for '
        //             + zonename);
        //         return;
        //     }
        // }
        ...
        self.event_queue.enqueue(zonename, function (callback) {
            ...
            if (!self.vmobjs.hasOwnProperty(zonename)) {
                ...
                // Fetching zoneinfo objects is really expensive... let's
                // try to minimize that code path when we don't need it.
                // Essentially, the zoneinfo_objects only need refreshed
                // when the zone reboots. By unsetting the value on
                // uninitialized, we will re-initialize the data the next
                // time the zone comes online.
                if (obj.newstate == 'uninitialized') {
                    contexts.push('zoneinfo_objects');
                }
        ...
        if (zonexml_re.test(obj.pathname)) {
            ...
            self.event_queue.enqueue(function (callback) {
                async.each(vmDatasets(), function (data, cb) {
                    ...
                    var opts = {log: self.log, cache: data};
    ...
    function handleZpoolEvent(obj) {
        ...
        self.event_queue.enqueue(uuid, function (cb) {
        ...
        async.waterfall([
            ...
            function (cb) {
                ...
                vmload.getZoneData(zonename, opts, function (err, cache) {
                    ...
                    // cache in this case is just a reference to vm_data, which
                    // we already store globally. The idea here is that we just
                    // fill in the holes that we previously created.
    ...
    function handleGet(c, args, response) {
        self.log.debug('GET (' + JSON.stringify(c) + ') len: ' + c.length);
        ...
        case 'events':
            ...
            var on_modify = function (vmobj, changes) {
                var data = {
                    type: 'modify',
                    zonename: vmobj.zonename,
                    vm: vmobj,
            ...
            function cleanup() {
                self.removeListener('create', on_create);
                self.removeListener('modify', on_modify);
                self.removeListener('delete', on_delete);
                self.removeListener('close', on_close);
            }

            self.on('create', on_create);
            self.on('modify', on_modify);
