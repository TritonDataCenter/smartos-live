...
function validateImage(image, log, callback)
{
    ...
    traceExecFile(cmd, args, log, 'imgadm-get',
        ...
        if (data.hasOwnProperty('manifest')) {
            ...
            // Check for virtio mode tag on image.
            var virtioTag = 'org.smartos:virtio';
            if (manifest.hasOwnProperty('tags')
                && manifest.tags.hasOwnProperty(virtioTag)) {
                image.virtio = manifest.tags[virtioTag];
                log.debug({virtio: image.virtio},
                    'found virtio tag on image ' + image.uuid);
            }
    ...
    async.forEachSeries(check_images, function (image, cb) {
        ...
        validateImage(i, log, function (err) {
            if (err) {
                ...
                // Copy virtio mode tag from image manifest to payload
                if (i.hasOwnProperty('virtio')) {
                    payload.virtio = i.virtio;
                }
            }
