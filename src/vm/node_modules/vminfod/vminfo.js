    ...
    function handleZoneEvent(obj) {
        ...
        self.event_queue.enqueue({
            ...
            job: function (callback) {
                self.log.debug('handling zone event for %s', zonename);
        ...
        if (zonexml_re.test(obj.pathname)) {
            ...
            self.event_queue.enqueue({
                ...
                job: function (callback) {
                    ...
                    // short-circuit if this vmobj doesn't exist
                    if (!self.vmobjs.hasOwnProperty(zonename)) {
                        self.log.debug('vmobj doesn\'t exist for %s', zonename);
                        callback();
                        return;
                    }
        ...
        self.event_queue.enqueue({
            ...
            job: function (cb) {
                ...
                if (uuid !== 'global') {
                    ...
                    self.log.debug('refreshing vmobj %s after zfs event', uuid);
...
VMInfo.prototype.start = function (callback) {
    ...
    async.series([
        ...
        function (cb) {
            ...
            var opts = {workers: 1, paused: true, log: self.log};
