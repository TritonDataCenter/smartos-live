...
var cloudinit = require('./cloudinit');
...
function validateImage(image, log, callback)
{
    ...
    traceExecFile(cmd, args, log, 'imgadm-get',
        ...
        if (data.hasOwnProperty('manifest')) {
            ...
            // Check for cloud-init datasource tag on image.
            var datasourceTag = 'org.smartos:cloudinit_datasource';
            if (manifest.hasOwnProperty('tags')
                && manifest.tags.hasOwnProperty(datasourceTag)) {
                image.cloudinit_datasource =
                    manifest.tags[datasourceTag];
                log.debug({cloudinit_datasource: image.cloudinit_datasource},
                    'found cloudinit datasource tag on image ' + image.uuid);
            }
    ...
    async.forEachSeries(check_images, function (image, cb) {
        ...
        validateImage(i, log, function (err) {
            if (err) {
                ...
                // Copy cloudinit_datasource tag from modified image manifest
                // to internal_metadata on the payload
                if (i.hasOwnProperty('cloudinit_datasource')) {
                    if (typeof (payload.internal_metadata) === 'object')  {
                        payload.internal_metadata.cloudinit_datasource =
                            i.cloudinit_datasource;
                    } else {
                        payload.internal_metadata = {
                            cloudinit_datasource: i.cloudinit_datasource
                        };
                    }
                }
            }
...
function createVM(payload, log, callback)
{
    ...
    async.series([
        ...
        }, function _createCloudInitNoCloudVolume(cb) {
            if (payload.create_only) {
                cb();
                return;
            }

            var internal_metadata = payload.internal_metadata;
            if (payload.brand !== 'bhyve' || !internal_metadata
                || internal_metadata.cloudinit_datasource !== 'nocloud'
                || internal_metadata.cloudinit_nocloud_path) {
                cb();
                return;
            }

            cloudinit.nocloud.updatePayloadDisks(log, payload);
        ...
        }, function _storeCloudInitNoCloudVolumePath(cb) {
            var im = payload.internal_metadata;
            if (payload.brand !== 'bhyve' || !im
                || im.cloudinit_datasource !== 'nocloud') {
                cb();
                return;
            }

            // Store the CloudInit volume's path in internal_metadata so it
            // can easily be identified later whenever the filesystem needs
            // to be regenerated upon reboot (For example: when adding or
            // removing a NIC.)
            // How guests handle a CloudInit filesystem update varies wildly,
            // some guests ignore it entirely, some always reprocess, some
            // require the instance-id to change.
            for (var i = 0; i < payload.add_disks.length; i++) {
                if (payload.add_disks[i].cloudinit_datasource === 'nocloud'
                    && payload.add_disks[i].path) {
                    payload.internal_metadata.cloudinit_nocloud_path =
                        payload.add_disks[i].path;
                    log.debug({path: payload.add_disks[i].path},
                        'Stored CloudInit volume path in internal_metadata');
                    break;
                }
            }
            cb();
...
function applyZoneDefaults(payload, log)
{
    ...
    if (allowed.hasOwnProperty('virtio1')
        && !payload.hasOwnProperty('virtio1')) {
        // New BHYVE VMs (only time this gets called) get virtio1 set to true.
        // See proptable.js, vmadm.8.md, and OS-8723 for more.
        payload.virtio1 = true;
    }
...
//
/*
 * For bhyve live migration, the destination-side VM is booted in a
 * migrate-listen mode where bhyve skips bootrom and vcpu_reset and
 * blocks on its in-zone control socket waiting for import-state.
 *
 * The zone-brand boot program (usr/src/lib/brand/bhyve/zone/boot.c)
 * reads the `migrate_listen` zone attribute at boot time and appends
 * `-o migrate.listen=true` to the bhyve argv when set.  We set the
 * attribute transiently — just for the window of a single
 * `zoneadm boot` call — and clear it immediately after so an
 * incidental later reboot (SMF restart, host reboot with autoboot,
 * anything) does not re-enter listen mode.
 *
 * Setting via `zonecfg add attr; commit` and clearing via
 * `zonecfg remove attr; commit` is the simplest write path that does
 * not require a persistent schema change.  If a subsequent boot-time
 * cleanup fails to run (e.g. startZone crashes mid-call), an operator
 * can verify with `zonecfg -z <uuid> info attr name=migrate_listen`
 * and remove it manually.
 */
function setMigrateListenAttr(uuid, enabled, log, callback)
{
    // zonecfg attr names disallow '_'; zoneadmd converts '-' to '_' when
    // exporting the attr as an _ZONECFG_attr_<name> env var to the brand
    // boot program.  So the on-disk attr name is migrate-listen but the
    // brand-side C code reads it as `migrate_listen`.
    var stdin = enabled
        ? 'add attr; set name=migrate-listen; set type=string; '
          + 'set value=true; end; commit\n'
        : 'remove attr name=migrate-listen; commit\n';

    zonecfg(uuid, [], {log: log, stdin: stdin}, function (err, fds) {
        if (err) {
            // Clearing a non-existent attr is fine; report-and-continue.
            if (!enabled
                && /No such attr resource/.test(fds.stderr || '')) {
                log.debug('migrate_listen attr already absent; ok');
                callback();
                return;
            }
            log.error({err: err, stdout: fds.stdout, stderr: fds.stderr},
                'failed to ' + (enabled ? 'set' : 'clear')
                + ' migrate_listen attr');
            callback(err);
            return;
        }
        log.debug({stdout: fds.stdout, stderr: fds.stderr},
            (enabled ? 'set' : 'cleared') + ' migrate_listen attr');
        callback();
    });
}
...
function startZone(vmobj, opts, callback)
{
    ...
    assert.optionalBool(opts.migrate_listen, 'opts.migrate_listen');
    ...
    var waited = false;
    var waitErr;
    var migrate_listen = !!opts.migrate_listen;
    ...
    async.series([
        function (cb) {
            ...
            // bhyve migrate-listen: set the zone attribute just before boot.
            // The cleanup step below clears it after zoneadm returns so an
            // incidental reboot doesn't re-enter listen mode.
            if (!migrate_listen) {
                cb();
                return;
            }
            setMigrateListenAttr(uuid, true, log, cb);
        }, function (cb) {
            // do the booting
            var cancelFn;
            ...
            vasync.parallel({funcs: [
                function (cb2) {
                    ...
                    // Wrap zoneadm boot so the migrate_listen attribute is
                    // cleared whether boot succeeds or fails — matches the
                    // "transient for this boot call only" contract.
                    function finishBoot(err, boot_fds) {
                        if (!migrate_listen) {
                            cb2(err);
                            return;
                        }
                        setMigrateListenAttr(uuid, false, log,
                            function (clear_err) {
                            if (clear_err) {
                                log.warn({err: clear_err},
                                    'failed to clear migrate_listen attr; '
                                    + 'operator may need to remove it '
                                    + 'manually via zonecfg');
                            }
                            cb2(err);
                        });
                    }
...
// zonepath
//
function startVM(vmobj, extra, opts, log, callback)
{
    assert.object(opts, 'opts');
    assert.optionalBool(opts.migrate_listen, 'opts.migrate_listen');
    var check_path;
    ...
    fs.writeFile(path.join(vmobj.zonepath, '/root/tmp/vm.metadata'),
        ...
        function (err) {
            if (err) {
                ...
                startZone(vmobj,
                    {log: log, migrate_listen: opts.migrate_listen},
                    callback);
            }
        }
    );
    ...
    async.series([
        function _loadVM(cb) {
            ...
            if (options.vmobj && options.vmobj.uuid === uuid) {
                ...
                vmobj = options.vmobj;
                vm_type = BRAND_OPTIONS[vmobj.brand].features.type;
                cb();
                return;
            }
        ...
        }, function _loadHVM(cb) {
            // When we boot KVM or bhyve we need additional fields and must
            // call VM.load() again. We can skip this step for LX and OS VMs.
            if (['BHYVE', 'KVM'].indexOf(vm_type) === -1) {
                cb();
                return;
            }
        ...
        }, function _createNoCloudFS(cb) {
            // We can only create a CloudInit NoCloud filesystem for HVM VMs
            // We can skip native VMs and HVM instances that don't use NoCloud
            var im = hvmobj && hvmobj.internal_metadata;
            var noCloudPath = im && im.cloudinit_nocloud_path;
            if (['BHYVE', 'KVM'].indexOf(vm_type) === -1
                || !im
                || !noCloudPath
                || im.cloudinit_datasource !== 'nocloud') {
            ...
            if (!cloudinit.nocloud.validateNoCloudDisk(
                hvmobj.disks, noCloudPath)) {
                cb(new Error('Invalid cloudinit_nocloud_path: ' + noCloudPath));
                return;
            }

            var cloudInitPayload = {
                uuid: hvmobj.uuid,
                hostname: hvmobj.hostname,
                alias: hvmobj.alias,
            ...
            var newHash = crypto.createHash('md5')
                ...
                .digest('hex');
            var storedHash = im.cloudinit_nocloud_hash;

            if (newHash === storedHash) {
                log.debug('NoCloud payload unchanged (hash %s), skipping '
                    + 'filesystem rebuild', newHash);
                cb();
                return;
            }

            cloudinit.nocloud.createPCFS(log, cloudInitPayload, noCloudPath,
                function _noCloudFSCreated(ciErr) {
                if (ciErr) {
                    log.error({err: ciErr},
                        'Failed to generate NoCloud filesystem');
                    cb(ciErr);
                    return;
                }
                log.debug('Generated NoCloud filesystem');

                im.cloudinit_nocloud_hash = newHash;
                writeAllMetadata(hvmobj.uuid, hvmobj.zonepath,
                    {internal_metadata: im},
                    {log: log, skipLockfile: true},
                    function _hashWritten(mErr) {
                    if (mErr) {
                        log.warn({err: mErr},
                            'Failed to persist payload hash. Next boot will '
                            + 'rebuild the NoCloud filesystem');
                    }
                    cb();
                });
            });
        }, function _startVM(cb) {
            var err;

            if (['BHYVE', 'KVM'].indexOf(vm_type) !== -1) {
                // migrate_listen is a bhyve-only flag — propagate into
                // startVM so the destination-side of a live migration
                // boots with -o migrate.listen=true on the bhyve argv.
                // Harmless (ignored) on KVM paths.
                startVM(hvmobj, extra,
                    {migrate_listen: !!options.migrate_listen},
                    log, cb);
            } else if (['OS', 'LX'].indexOf(vm_type) !== -1) {
                startZone(vmobj, {
                    log: log,
