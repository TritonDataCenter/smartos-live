 ...
 * media(uuid, req=[eject|change], options={}, callback)
 ...
 * MEDIA_CMDS - list of supported commands for media
 */
...
// Media commands with number of extra args
exports.MEDIA_CMDS = [
    'eject',
    'change'
];
        ...
        if (obj.brand === 'kvm' && obj.hasOwnProperty('disks')) {
            for (disk in obj.disks) {
                ...
                if (preload_data.hasOwnProperty('dsinfo') &&
                    ...
                    disk.zfs_filesystem = preload_data.dsinfo[disk.path].dataset;
...
function setQuota(dataset, quota, callback)
{
    ...
    if (!dataset) {
        return callback(new Error('Invalid dataset: "' + dataset + '"'));
    }
    ...
    getVmobj(info.zonename, data, function (err, vmobj) {
        ...
        loadMetadata(vmobj, function (err, metadata, tags) {
            ...
            if (vmobj.hasOwnProperty('zfs_filesystem')) {
                vmobj.zfs_storage_pool_name = vmobj.zfs_filesystem.split('/')[0];
            }
        ...
        if (payload.add_nics.hasOwnProperty(nic)) {
            ...
            if (n.hasOwnProperty('primary') && !primary_found) {
                ...
                if (n.hasOwnProperty('gateway')) {
                    fs.writeFileSync(payload.zonepath + '/root/etc/defaultrouter',
                        n.gateway + '\n');
                }
                if (n.ip == 'dhcp') {
        ...
        for (disk in payload.disks) {
            if (payload.disks.hasOwnProperty(disk)) {
                ...
                if (zvol.hasOwnProperty('zpool') &&
                    ...
                    return callback(new Error('invalid zpool name: ' +
                        zvol.zpool));
                }
                ...
                if (payload.brand === 'kvm') {
                    if (!zvol.hasOwnProperty('model') ||
                        ...
                        return callback(new Error('missing .model option for ' +
                            'disk: ' + JSON.stringify(zvol)));
                    } else if (VM.DISK_MODELS.indexOf(zvol.model) === -1) {
                        return callback(new Error('"' + zvol.model + '"' +
...
function startVM(vmobj, extra, callback)
{
    ...
    if (extra.hasOwnProperty('disks')) {
        for (disk in extra.disks) {
            if (extra.disks.hasOwnProperty(disk)) {
                ...
                console.log('checking: ' + check_path);
                if (!found && path.existsSync(check_path)) {
                    found = true;
                }
                if (!found) {
                    return callback(new Error('Cannot find disk: ' +
                        disk.path));
                }
    ...
    if (!vmobj.qemu_opts) {
	if(vmobj.hasOwnProperty('vnc') && vmobj.vnc == 'secure') {
...
function postVmadmd(uuid, action, args, callback)
{
    ...
    req = http.request({ method: 'POST', socketPath: VMADMD_SOCK, path: path }, function(res) {
...
exports.info = function (uuid, types, callback)
{
    ...
    VM.load(uuid, function (err, obj) {
        ...
        http.get({ socketPath: VMADMD_SOCK, path: '/vm/' + uuid + '?types=' + types.join(',') }, function(res) {
            data = '';
...
exports.media = function(uuid, req, options, callback)
{
    VM.syslog_action = 'media-' + req;

    VM.log('DEBUG','media(' + uuid + ',' + req + ')');
    ...
    VM.load(uuid, function (err, obj) {
        ...
        if (obj.brand !== 'kvm') {
            return callback(new Error('The media command is only supported for'
                + ' VMs.'));
        }

        if (VM.MEDIA_CMDS.indexOf(req) === -1) {
            return callback(new Error('Invalid media command "' + req + '" valid values: '
                + '"' + VM.MEDIA_CMDS.join('","') + '".'));
        }

	options.command = req;
        postVmadmd(obj.uuid, 'media', options, function (err) {
            if (err) {
                return callback(new Error('Unable to post "media" to vmadmd'));
            }
            return callback();
        });
    });
