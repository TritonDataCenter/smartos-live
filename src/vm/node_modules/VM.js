...
function createVM(payload, log, callback)
{
    ...
    async.series([
        ...
        }, function _createNoCloudVolume(cb) {
            ...
            for (var i = 0; i < payload.add_disks.length; i++) {
                if (payload.add_disks[i].cloudinit_datasource === 'nocloud'
                    ...
                    log.debug({path: payload.add_disks[i].path},
                        'Stored CIDATA path in internal_metadata');
                    break;
                }
            }
            cb();
...
function installZone(payload, log, callback)
{
    ...
    async.series([
        function (cb) {
            ...
            // Look for a disk with a cloudinit_datasource prop added in
            // createVM(), it must have the value of 'nocloud'.
            var targetZvol = null;
            if (payload.add_disks) {
                for (var i = 0; i < payload.add_disks.length; i++) {
                    var add_disk = payload.add_disks[i];
                    if (!add_disk.boot
                        && add_disk.cloudinit_datasource === 'nocloud'
                        && add_disk.path) {
                        targetZvol = add_disk.path;
                        break;
                    }
                }
            }
            ...
            if (targetZvol) {
                cloudinit.nocloud.createVolume(log, payload, targetZvol, cb);
                return;
            }
        ...
        function _loadVM(cb) {
            ...
            if (['BHYVE', 'KVM'].indexOf(vm_type) !== -1) {
                ...
                VM.load(uuid, {
                    ...
                    im = obj.internal_metadata;
                    noCloudPath = im && im.cloudinit_nocloud_path;

                    // Regenerate CIDATA for nocloud VMs before starting
                    if (im && noCloudPath
                        && im.cloudinit_datasource === 'nocloud') {

                        log.debug({path: noCloudPath},
                            'Generating CIDATA for nocloud VM');

                        // Sanity check: ensure that obj.disks contains
                        // the noCloudPath we have in internal_metadata and
                        // that it's shaped like a CIDATA zvol.
                        for (var i = 0; i < obj.disks.length; i++) {
                            if (obj.disks[i].path === noCloudPath
                                && obj.disks[i].size === imgSize
                                && obj.disks[i].block_size === blockSize
                                && obj.disks[i].boot === false) {
                        ...
                        if (!foundNoCloudPath) {
                            cb(new Error('Invalid cloudinit_nocloud_path: '
                                + noCloudPath));
                            return;
                        }
                        ...
                        cloudinit.nocloud.createVolume(log, cidataPayload,
                            im.cloudinit_nocloud_path,
                            function _nocloudCreateVolume(ciErr) {
                            if (ciErr) {
                                log.error({err: ciErr},
                                    'Failed to regenerate CIDATA');
                                cb(ciErr);
                                return;
                            }
                            log.debug('Regenerated CIDATA');
