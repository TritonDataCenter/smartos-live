...
// when a file fails to be watched during its own short watch interval it
...
FsWatcher.prototype.start = function start() {
    ...
    assert.ok(!self.running(), 'already running');

    self.shutting_down = false;
    self.stderr_buffer = new RingBuffer({limit: 10});
    ...
    self.watcher = cp.spawn(FSWATCHER_CMD, [], {stdio: 'pipe'});
    ...
    self.watcher.on('close', function (code, signal) {
        ...
        if (self.shutting_down) {
            ...
            self.log.error({err: e, signal: signal, code: code, pid: e.pid},
                'fswatcher exited');
            self.log.error({not_yet_watching: self.not_yet_watching},
                'fswatcher not_yet_watching');
            self.log.error({watching: self.watching},
                'fswatcher watching');
    ...
    function handleEvent(obj, cb) {
        ...
        if (obj.type === 'delete') {
            self.log.debug({f: f}, 'file was deleted, queueing rewatch');
    ...
    function handleResponse(obj, cb) {
        ...
        assert.string(obj.pathname, 'obj.pathname');
        assert.string(obj.message, 'obj.message');
...
FsWatcher.prototype.stop = function stop() {
    ...
    assert.ok(self.running(), 'not running');
...
FsWatcher.prototype.shutdown = FsWatcher.prototype.stop;

// check if currently running
FsWatcher.prototype.running = function running() {
    var self = this;

    return !!(self.watcher && self.watcher_pid && self.long_watch_timeout);
};
