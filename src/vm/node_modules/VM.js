...
function createVM(payload, log, callback)
{
    ...
    async.series([
        function (cb) {
            ...
            var diskSizeMiB = cloudinit.nocloud.DISK_SIZE_MIB;
            ...
            payload.add_disks.push({
                ...
                boot: false,
                media: 'disk',
                block_size: cloudinit.nocloud.BLOCK_SIZE,
...
exports.start = function (uuid, extra, options, callback)
{
    ...
    async.series([
        function _loadVM(cb) {
            ...
            var foundNoCloudPath = false;
            var imgSize = cloudinit.nocloud.DISK_SIZE_MIB;
            var blockSize = cloudinit.nocloud.BLOCK_SIZE;

            // Sanity check: ensure that hvmobj.disks contains
            // the noCloudPath we have in internal_metadata and
            // that it's shaped like a CloudInit zvol.
            for (var i = 0; i < hvmobj.disks.length; i++) {
                if (hvmobj.disks[i].path === noCloudPath
                    && hvmobj.disks[i].size === imgSize
                    && hvmobj.disks[i].block_size === blockSize
                    && hvmobj.disks[i].boot === false) {
                    foundNoCloudPath = true;
                    break;
                }
            }
