...
function VMInfo(options) {
        ...
        self.event_queue.enqueue({
            ...
            func: function (extras, callback) {
                if (!self.vmobjs.hasOwnProperty(zonename)) {
                    ...
                    invalidateZoneDataCache(zonename, vm_datasets, contexts);
    ...
    function handleFsEvent(obj) {
        ...
        var vm_datasets;
        var contexts;
        var meta = util.format('pathname: %s (%s)',
            ...
            self.event_queue.enqueue({
                ...
                func: function (extras, callback) {
                    ...
                    case 'change':
                        ...
                        invalidateZoneDataCache(zonename, vm_datasets,
                            contexts);
                        refreshVmobj(zonename, vm_datasets, callback);
                        break;
                    default:
    ...
    function refreshVmobj(zonename, vm_datasets, callback) {
        ...
        vasync.pipeline({funcs: [
            ...
            function (_, cb) {
                // We remove both VMs "last_modified" propery to compare the
     ...
     *
     */
    function addVMFsWatches(zonename, callback) {
        var files = [
            util.format('/etc/zones/%s.xml', zonename),
            util.format('/zones/%s/lastexited', zonename)
        ];
        CONFIG_FILES.forEach(function (f) {
            files.push(util.format('/zones/%s/config/%s', zonename, f));
        });
        ...
        vasync.forEachPipeline({
            ...
            self.log.debug({err: err},
                'finished adding vm fs watchers for %s', zonename);
            callback(err);
        });
    }
     ...
     *
     */
    function removeVMFsWatches(zonename, callback) {
        var files = [
            util.format('/etc/zones/%s.xml', zonename),
            util.format('/zones/%s/lastexited', zonename)
        ];
        ...
        vasync.forEachPipeline({
            func: function (f, cb) {
                self.fsw.unwatch(f, cb);
            },
            inputs: files
        }, function (err, results) {
            self.log.debug({err: err},
                'finished removing vm fs watchers for %s', zonename);
            callback(err);
        });
    }
    ...
    function serializeDataSysinfo() {
        if (!self.vm_data_json.hasOwnProperty('sysinfo')) {
            self.vm_data_json.sysinfo = JSON.stringify(self.vm_data.sysinfo);
        }
    ...
    function handleGet(c, args, request, response) {
        var res;
        var obj;
        var uuid;
        switch (c[0]) {
    ...
    self.startHTTPHandler = function (callback) {
        var handler = function (request, response) {
            ...
            if (request.method !== 'GET') {
                ...
                handleGet(c, args, request, response);
            }
        };
