    ...
    if (XML_PROPERTIES.hasOwnProperty(where)) {
        ...
        for (prop in attrs) {
            if (XML_PROPERTIES.nic.hasOwnProperty(prop)) {
                ...
                VM.logger('DEBUG', 'unknown net prop: ' + prop);
            }
        }
        ...
        if (XML_PROPERTIES.nic.hasOwnProperty(attrs.name)) {
            ...
            VM.logger('DEBUG', 'unknown net prop: ' + attrs.name);
        }
        ...
        for (prop in attrs) {
            if (XML_PROPERTIES.disk.hasOwnProperty(prop)) {
                ...
                VM.logger('DEBUG', 'unknown disk prop: ' + prop);
            }
        }
        ...
        if (XML_PROPERTIES.disk.hasOwnProperty(attrs.name)) {
            ...
            VM.logger('DEBUG', 'unknown disk prop: ' + attrs.name);
        }
    } else {
        VM.logger('DEBUG', 'unknown property: ' + where + ': ' +
    ...
    fs.readFile(filename, function (error, data) {
        ...
        if (obj.hasOwnProperty('transition')) {
            ...
            if (fields.length === 3) {
                ...
                VM.logger('DEBUG', 'getVmobj() ignoring bad value for ' +
                    'transition "' + obj.transition + '"');
            }
        }

        // TODO: sort nics + disks by index
    ...
    return zfs(args, function (err, fds) {
        ...
        for (line in lines) {
            ...
            if (fields.length === 2) {
                ...
                VM.logger('DEBUG', 'loadDiskQuotas(): ignoring line: ' + line);
            }
        }
    ...
    return zfs(args, function (err, fds) {
        ...
        for (line in lines) {
            ...
            if (fields.length === 2) {
                ...
                VM.logger('DEBUG', 'loadDatasetQuota(): ignoring line: ' + line);
            }
        }
...
function preloadZoneData(uuid, callback)
{
    ...
    async.series([
        ...
        function (cb)
        {
            ...
            async.forEach(uuids,
                function (uuid, zcb) {
                    ...
                    var filename;
                    VM.logger('DEBUG', 'here z:' + JSON.stringify(z));
                    ...
                    if (z.brand === 'kvm' && z.state === 'running') {
                        ...
                        VM.logger('DEBUG', 'checking for ' + filename);
                        path.exists(filename, function (exists) {
                            if (exists) {
                                fs.readFile(filename, function (error, data) {
                                    var pid;
                                ...
                                VM.logger('DEBUG', 'no PID file for ' + z.uuid);
                            }
                        });
                    } else {
                        return zcb();
        ...
        VM.logger('DEBUG', 'leaving preloadZoneData()');
...
exports.getFlattened = function(vmobj, key)
{
    ...
    getVmobj(info.zonename, function (err, vmobj) {
        ...
        loadMetadata(vmobj, function (err, metadata, tags) {
            ...
            if (data.dsquotas.hasOwnProperty(dataset)) {
                ...
                VM.logger('DEBUG', 'found quota "' + vmobj.quota + '" for ' +
                    vmobj.uuid);
            } else {
                VM.logger('DEBUG', 'no dsquota for ' + vmobj.uuid);
            }
            ...
            if (vmobj.hasOwnProperty('disks')) {
                for (d in vmobj.disks) {
                    ...
                    if (d.hasOwnProperty('zfs_filesystem') &&
                        ...
                        VM.logger('DEBUG', 'found size=' + d.size + ' for ' +
                            JSON.stringify(d));
                    }
                }
            }
...
function matcher(zone, search)
{
    ...
    function find_match(key, target) {
        var value = VM.getFlattened(zone, key);
...
function lookupConflicts(macs, ips, callback) {
    ...
    preloadZoneData(null, function (err, data) {
        ...
        async.forEach(records, function (vm, cb) {
            zonename = vm.zonename;
            loadVM(zonename, data, function (error, obj) {
                if (error) {
                    return cb(error);
                }
                for (ip in ips) {
            ...
            VM.logger('DEBUG', 'returning from conflict check');
...
function createVolumes(payload, callback)
{
    ...
    if (payload.hasOwnProperty('starting_index')) {
        disk_idx = payload.starting_index;
    } else {
        disk_idx = 0;
    }
    for (disk in payload.disks) {
...
function waitForJoyentZone(payload, callback)
{
    ...
    VM.logger('DEBUG', '/usr/vm/sbin/zoneevent');
    ...
    watcher.stdout.on('data', function (data) {
        ...
        while (chunks.length > 1) {
            ...
            if (obj.zonename === payload.zonename) {
                VM.logger('DEBUG', 'zoneevent said: ' + JSON.stringify(obj));
                ...
                if (!found && obj.newstate === 'running') {
                    VM.logger('INFO', payload.uuid + ' went back to "running"' +
    ...
    watcher.on('exit', function (code) {
        VM.logger('ERROR', 'zoneevent watcher ' + watcher_pid + ' exited: ', JSON.stringify(code));
        cleanup();
...
function checkProperties(payload, callback)
{
    ...
    VM.logger('DEBUG', 'looking up conflicts');
...
function createZone(payload, callback)
{
    ...
    async.series([
        function (cb) {
            ...
            zoneadm(args, function (err, fds) {
                if (err) {
                    VM.logger('ERROR', 'zoneadm failed to install: ' +
...
exports.start = function (uuid, extra, callback)
{
    ...
    VM.load(uuid, function (err, obj) {
        ...
        if (zoneobj.hasOwnProperty('brand') && zoneobj.brand === 'joyent') {
            ...
            return callback(new Error('no idea how to start a vm with brand: '
                + zoneobj.brand));
        }
        return callback();
    });
...
function resizeDisks(disks, updates, callback)
{
    ...
    async.forEach(vols, resize, function (err) {
        if (err) {
            VM.logger('ERROR', 'Unable to resize disks', err);
            return callback(err);
        }
        return callback();
    });
    ...
    async.series([
        function (cb) {
            if (payload.hasOwnProperty('update_disks') &&
                m.hasOwnProperty('disks')) {

                return resizeDisks(m.disks, payload.update_disks, cb);
            } else {
                return cb();
            }
        }, function (cb) {
...
{
    VM.logger('DEBUG', 'updating ' + uuid + ' with payload=' + JSON.stringify(payload));
            ...
            setTransition(obj, 'stopping', options.transition_to,
                ...
                function (err)
                {
                    ...
                    postVmadmd(obj.uuid, 'stop', {'timeout': options.timeout},
                        ...
                        {
                            if (err) {
                                return callback(new Error('Unable to post ' +
                                    '"stop" to vmadmd'));
                            }
                            zonecfg(['-u', uuid, unset_autoboot], function (e, fds) {
                                if (err) {
