...
function FsWatcher(options)
{
    ...
    self.startWatcher = function () {
        ...
        self.watcher.on('close', function (code, signal) {
            ...
            log.error({err: e, signal: signal, code: code, pid: e.pid},
                'fswatcher exited');
            if (!self.shutting_down) {
                log.error({pending_actions: self.pending_actions},
                    'fswatcher pending actions');
                log.error({watches: self.watches},
                    'fswatcher watches');
                self.emit('error', e);
            }
    ...
    self.res_queue = vasync.queue(function (res, callback) {
        ...
        log.trace({obj: obj}, 'fswatcher event');
...
FsWatcher.prototype.dump = function () {
    var self = this;

    return {
        pid: self.watcher_pid,
        watches: Object.keys(self.watches).sort(),
        pending_actions: self.pending_actions
    };
};
