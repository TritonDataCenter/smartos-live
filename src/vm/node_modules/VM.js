...
exports.waitForZoneState = function (payload, state, options, callback)
{
    ...
    function handleEvent(task) {
        if (task.zonename == payload.zonename) {
            if (task.hasOwnProperty('vm')) {
                if (task.vm.hasOwnProperty('zone_state')
                    && task.vm.zone_state === state) {
    ...
    request = http.get(opts, function (res) {
        ...
        res.on('close', function () {
            ...
            callback(new Error('Events channel closed prematurely'));
        });
    }).on('error', function (err) {
        done();
        log.error('vminfod request failed: ' + err.message);
        callback(new Error('Unable to subscribe to vminfod events'));
    });
