    ...
    function handleGet(c, args, req, res) {
        ...
        case 'status':
            ret = {
                ...
                vms: Object.keys(self.vmobjs).length,
                state: self.state,
                status: self.status,
 ...
 */
Vminfo.prototype.stop = function stop(callback) {
    var self = this;
...
Vminfo.prototype.reset = function reset(callback) {
    ...
    vasync.pipeline({funcs: [
        ...
        var prettyTime = hrtime.hrtimeToString(delta);
        var prettyDelta = hrtime.prettyHrtime(delta);

        self.log.debug('self.reset() took %ss (%s)', prettyTime, prettyDelta);
