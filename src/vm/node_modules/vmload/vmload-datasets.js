...
function getZfsList(options, callback) {
    ...
    if (options.hasOwnProperty('dataset')) {
        task.dataset = options.dataset;
    }

    // we have to stringify the object and use that as a key, because
    // if we just use the object directly, all objects evaluate to the
    // same key when accessing an object property.
    key = task.types.join(',') + ',' + task.fields.join(',');
    if (task.hasOwnProperty('dataset')) {
        key += ',' + task.dataset;
    }
...
function zfsList(options, callback) {
    ...
    // if a specific dataset was passed, limit the zfs list to that dataset
    if (options.hasOwnProperty('dataset')) {
