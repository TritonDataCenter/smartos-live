    ...
    if (opts.timeout) {
        timeout = setTimeout(function () {
            ...
            self.vs_logger.error({obj: obj, events: events},
                'vminfod matchForEvent timeout exceeded - unmatched events');
...
function watchForChanges(obj, changes, opts, cb) {
    ...
    changes.forEach(function (change, i) {
        assert.string(change.path,
            util.format('changes.%d.path', i));
    });
        ...
        while ((ev = self.read()) !== null) {
            ...
            (ev.changes || []).forEach(function (change) {
                ...
                changes = changes.filter(function (wanted) {
                    var originalWantedPath = wanted.path;
                    var match = true;

                    var wantedPath = wanted.path.split('.');
                    var changePath = change.path.split('.');
                    ...
                    if (wantedPath.length !== changePath.length) {
                        ...
                        for (var i = 0; i < wantedPath.length; i++) {
                            var wantedPart = wantedPath[i];
                            var changePart = changePath[i];
                            if ( ! (wantedPart === '*' ||
                    ...
                    if (match) {
                        ...
                        // not a match, store this event for debugging
                        events.push(change);
                    }
